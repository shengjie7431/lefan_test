    ...
    <div class="panel panel-info">
        ...
        <table class="table table-hover">
            ...
            <tr>
                ...
                <th width="150">商品code</th>
                <th width="100">商品价格</th>
                <th width="100">商品乐凡币</th>
                ...
                <tr>
                    <td>
                        <c:if test="${item.proFile !=null}">
                            <img src="${item.proFile}" width="75;" height="75;" class="picToBig">
                        </c:if>
                    </td>
                    <td>${item.name}</td>
                    <td>${item.code}</td>
                    <td>${item.price}</td>
                    <td>${item.lefanCoin}</td>
                    <td>
                        <c:if test="${item.isUp == 0}">否</c:if>
                        <c:if test="${item.isUp == 1}">是</c:if>
                    </td>
                    <td><fmt:formatDate value="${item.upTime}" pattern="yyyy-MM-dd HH:mm"/></td>
