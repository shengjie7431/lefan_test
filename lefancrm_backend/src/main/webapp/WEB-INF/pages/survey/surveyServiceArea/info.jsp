...
<div class="main">
    ...
    <div class="title">
        <button class="butList active" onclick="operate('${surveyServiceArea.id}','${surveyCode}','1000',false);">修改</button>
        <%--<button class="butList defuelt" onclick="operate('${surveyServiceArea.id}','${surveyCode}','9999',true);">删除</button>--%>
    ...
    <div class="main-boy">
        <div>
            <table class="spec-info" border="0" cellpadding="0" cellspacing="0">
                ...
                <tr>
                    <td>名称</td>
                    <td>${surveyServiceArea.name}</td>
                    <td>区域类型</td>
                    <td>
                        <c:if test="${surveyServiceArea.type==1}">省</c:if>
                        <c:if test="${surveyServiceArea.type==2}">市</c:if>
                        <c:if test="${surveyServiceArea.type==3}">区</c:if>
                    </td>
                    <td>区域名称</td>
                    <td>${surveyServiceArea.areaName}</td>
                </tr>
                <tr>
                    <td>区域地图</td>
                    <td><img src="${surveyServiceArea.areaMap}" width="75;" height="75;" class="picToBig"></td>
                    <td>创建时间</td>
                    <td><fmt:formatDate value="${surveyServiceArea.createTime}" pattern="yyyy-MM-dd HH:mm"/></td>
                    <td>更新时间</td>
                    <td><fmt:formatDate value="${surveyServiceArea.updateTime}" pattern="yyyy-MM-dd HH:mm"/></td>
                </tr>
                </tbody>
            </table>
        </div>
...
<div class="container">
        <div class="form-group">
            <table class="table">
                <tbody>
                <tr>
                    <th class="active"><strong class="necessary"> </strong>区域描述</th>
                    <td></td>
                </tr>
                <tr>
                    <td colspan="2">
