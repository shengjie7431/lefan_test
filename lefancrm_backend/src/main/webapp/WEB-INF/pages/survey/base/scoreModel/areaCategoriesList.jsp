    ...
    <div class="panel panel-info">
        <form id="editForm" role="form" action="${ctx}/baseSurvey/operate" method="post">
            ...
            <table class="table table-hover">
                <thead>
                <button onclick="add('${id}','scoreModelArea')" type="button" class="btn btn-default">设置区域分值系数</button>
                <button onclick="addPrice('${id}','scoreModelPrice','${type}')" type="button" class="btn btn-default">设置价格</button>
                <tr>
                ...
                <c:forEach items="${infos}" var="item">
                    <tr>
                        ...
                        <td>
                            <a href="javascript:operate('${item.id}','scoreModelArea','1000',false);">修改</a>

                            <a href="javascript:operate('${item.id}','scoreModelAreaDel','9999',true);">删除</a>
                        </td>
                    </tr>
                </c:forEach>
                </tbody>
...
<script type="text/html" id="export-content">
    <div class="layui-form">
        ...
        <div class="layui-inline export-btn">
            ...
            <label class="layui-form-label">
                <button type="button" class="layui-btn s-btn layui-btn-normal" data-type="export">生成区域分值</button>
            </label>
        </div>
    </div>
</script>
...
<script type="text/javascript">
    ...
    var add = function(id,surveyCode){
        openDialog({
            ...
            url:"${ctx}/baseSurvey/add?surveyCode="+surveyCode+"&modelId="+id
        });
    }
    ...
    function operate(id,surveyCode,btnCode,ajax){
        ...
        if(ajax){
            ...
            if(btnCode == '1000'){
                ...
                width = $(document.body).outerWidth() - 20;
                title = '修改';
                url = "${ctx}/baseSurvey/edit?id="+id+"&surveyCode="+surveyCode+"&modelId="+'${id}'
            }
            openDialog({
                frame:true,
                title:title,
    ...
    var addPrice = function(id,surveyCode,type){
        openDialog({
            ...
            height:800,
            width:$(document.body).outerWidth() - 20,
            url:"${ctx}/baseSurvey/edit?surveyCode="+surveyCode+"&modelId="+id+"&type="+type
        });
    }
    layui.config({
        ...
        $('.copyPrice').on('click', function () {
            ...
            $('.export-btn button').click(function () {
                ...
                if (_this.attr('data-type') == 'cancel') {
                    ...
                    var url = "${ctx}/baseSurvey/operate",param = {"modelId":$("#id").val(),"surveyCode":"copyPrice","selPriceModelId":exportDemo.getValue('valueStr')};
                    if(confirm('是否确认？')){
                        ajaxSubmit(url,param,function(v,e,p){
                            location.reload();
                        })
