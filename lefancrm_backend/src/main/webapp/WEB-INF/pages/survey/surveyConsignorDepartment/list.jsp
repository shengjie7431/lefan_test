...
<html>
<head>
    <title>委托方机构部门列表</title>
    <%@ include file="/WEB-INF/pages/common/mainCss.jsp" %>
</head>
<body>
<div class="main administrator">
    <div class="main-top">
        <h3>委托方机构部门列表 <small>共<span>${apiRsp.count}</span>个</small></h3>
    </div><!--main-top-->
            ...
            <div class="pin">
                <form class="form-inline" role="form" action="${ctx}/baseSurvey/list" method="post">
                    ...
                    <div class="form-group">
                       部门名称:<input name="name" type="text" value="${name}" class="form-control">
                    </div>
                    <div class="btn-group">
                        &nbsp; &nbsp;<button id="batchOperateBtn" type="submit" class="btn btn-default">查询</button>&nbsp; &nbsp;
                        &nbsp; &nbsp;<button onclick="add('${surveyCode}','${consignorId}')" type="button" class="btn btn-default">添加</button>&nbsp; &nbsp;
                    </div>
                </form>
            </div>
        </div>
        ...
        <table class="table table-hover">
            ...
            <c:forEach items="${apiRsp.results}" var="item">
                <tr>
                    ...
                    <td>
                        <a href="javascript:operate('${item.id}','${surveyCode}','1200',false);">
                            ${item.name}
                        </a>
                    </td>
                    <td>${item.createBy}</td>
                    ...
                    <td>${item.sort}</td>
                    <td>
                        <a href="javascript:operate('${item.id}','${surveyCode}','1100',false);">添加人员</a>
...
<script>
    ...
    var add = function(surveyCode,consignorId){
        openDialog({
            frame:true,
            title:"添加",
            height:500,
            width:800,
            url:"${ctx}/baseSurvey/add?surveyCode="+surveyCode+"&consignorId="+consignorId
        });
    }
...
//                        reloadParent();//删除时，需刷新父级
            ...
            if(btnCode == '1000'){
                ...
                width = 800;
                title = '修改';
                url = "${ctx}/baseSurvey/edit?id="+id+"&surveyCode="+surveyCode+"&consignorId="+${consignorId};
            }else if(btnCode == '1100'){
                height = 500;
                width = 850;
                title = '添加人员';
                url = "${ctx}/baseSurvey/popup?id="+id+"&surveyCode="+surveyCode+"&btnCode="+btnCode+"&consignorId="+${consignorId};
