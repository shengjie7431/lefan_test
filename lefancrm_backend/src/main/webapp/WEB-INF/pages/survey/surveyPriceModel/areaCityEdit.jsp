...
<div class="container">
    <form id="editForm">
        <input type="hidden" name="id" value="${priceModelAreaCategories.id}">
        <input type="hidden" name="priceModelId" value="${priceModelId}">
        ...
        <div class="form-group">
            <table class="table">
                ...
                <tr>
                    <th width="10%" class="active">区域类别名称</th>
                    <td width="90%">
                        <input type="text" id = "name" name="name" value="${priceModelAreaCategories.name}" class="form-control" required>
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td width="90%">
<%--                        <c:forEach items="${priceModelAreaCategories.areaCitys}" var="areaCitys">--%>
<%--                            ${areaCitys.areaName}--%>
<%--                        </c:forEach>--%>
                        <div class="areaBtns">
...
<script type="text/javascript">
    ...
    $(".btn-success").click(function () {
        ...
        layer.confirm('确认提交？',function () {
            if (_flag){
                _flag = false
                layer.msg("数据量过大，后台正在保存数据，请稍后刷新页面查看...",{icon:0,time:1000},function () {
                        submit();
                })
                setTimeout(function () {
                    reloadParent();
    ...
    function submit(){
        ...
        $('.areaInfo').map(function (i,cur) {
            ...
            $.ajax({
                url: '${ctx}/baseSurvey/update',
                data: {
                    id:$("input[name=id]").val(),
                    priceModelId: $("input[name=priceModelId]").val(),
                    surveyCode: $("input[name=surveyCode]").val(),
                    cityIds: newIds.join(','),
    ...
    layui.use(['layer','jquery'], function () {
        ...
        $('.areaBtns').on('click', '.areaInfo',function () {
            ...
            layerIndex2 = layer.open({
                ...
                success: function () {
                    $.ajax({
                        ...
                        data:{
                            areaId:areaId,
                            priceModelCaId: $("input[name=id]").val(),
                            surveyCode:"commonArea",
                            btnCode:"2000",
                            modelId:$("input[name=priceModelId]").val()
                        },
    ...
    $("#editForm").bind('submit', function(event) {
        ...
        layer.confirm('确认提交？',function () {
            ajaxFormSubmit(_this,null,null,null,null);
            layer.msg("正在保存数据...",{icon:0},function () {
                reloadParent();
            })
