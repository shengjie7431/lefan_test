...
<div class="main">
    <input type="hidden" name="id" value="${surveyAchieveRule.id}">
    <input type="hidden" name="surveyCode" value="${surveyCode}">
    <div class="title">
        <button class="butList active" onclick="operate('${surveyAchieveRule.id}','${surveyCode}','1000',false);">修改</button>
        <button class="butList defuelt" onclick="operate('${surveyAchieveRule.id}','${surveyCode}','9999',true);">删除</button>
    ...
    <div class="main-boy">
        <div>
            <table class="spec-info" border="0" cellpadding="0" cellspacing="0">
                ...
                <tr>
                    <td>规则code</td>
                    <td>${surveyAchieveRule.code}</td>
                    <td>规则名称</td>
                    <td>${surveyAchieveRule.name}</td>
                    <td>成就点数</td>
                    <td>${surveyAchieveRule.num}</td>
                </tr>
                <tr>
                    <td>说明</td>
                    <td colspan="6">${surveyAchieveRule.remark}</td>
                </tr>
                <tr>
                    <td>创建人</td>
                    <td>${surveyAchieveRule.createByName}</td>
                    <td>创建时间</td>
                    <td><fmt:formatDate value="${surveyAchieveRule.createTime}" pattern="yyyy-MM-dd HH:mm"/></td>
                    <td>更新时间</td>
                    <td><fmt:formatDate value="${surveyAchieveRule.updateTime}" pattern="yyyy-MM-dd HH:mm"/></td>
                </tr>
                </tbody>
            </table>
        </div>
