    ...
    layui.use(['form', 'table', 'soulTable', 'xmSelect', 'laydate'], function () {
        ...
        function setTable(_cols, param) {
            ...
            var myTable = table.render({
                ...
                parseData: function (res) {
                    resultsTotal = {
                        "platformApplicationIncomeRate": res.results.platTotalRate,
                        "insuranceCompanyConfirmsRevenueRate": getRate(res.results.invTotalRate),
                        "surveyCompanyConfirmsRevenue":getRate(res.results.surTotal),
                        "surveyCompanyConfirmsRevenueRate":getRate(res.results.surTotalRate),
                    }
                    return {
                        "code": res.isSuccess ? 0 : 1,
                        "msg": res.msg,
                        ...
                        "totalRow": {
                            ...
                            "insuranceCompanyConfirmsRevenueRate": getRate(res.results.invTotalRate),
                            "surveyCompanyConfirmsRevenue":res.results.surTotal,
                            "surveyCompanyConfirmsRevenueRate":getRate(res.results.surTotalRate),
        ...
        function showTitle(thsJ, child, iter) {
            ...
            // var _title3 = '平台复审通过时间在'+startTime+ '至' + endTime +'的保司案件中，各机构向委托方申请的对应委托方价格之和'
            ...
            if (child){
                ...
                var _title7 ='打卡时间在'+startTime+ '至' + endTime +'的保司案件中，机构内的调查员录入的市内交通费报销金额'
                ths.eq(0).find('.layui-table-cell').attr('title', _title7)

                var _title8 = '打卡时间在'+startTime+ '至' + endTime +'的保司案件中，机构内的调查员录入的病史费金额'
                ths.eq(1).find('.layui-table-cell').attr('title', _title8)

                var _title9 = '打卡时间在'+startTime+ '至' + endTime +'的保司案件中，机构内的调查员录入的住院排查费用金额'
                ths.eq(2).find('.layui-table-cell').attr('title', _title9)

                var _title99 = '打卡时间在'+startTime+ '至' + endTime +'的保司案件中，机构内的调查员录入的门诊排查费用金额'
                ths.eq(3).find('.layui-table-cell').attr('title', _title99)

                var _title10 = '打卡时间在'+startTime+ '至' + endTime +'的保司案件中，机构内的调查员录入的体检报告打印费金额'
                ths.eq(4).find('.layui-table-cell').attr('title', _title10)

                var _title11 = '打卡时间在'+startTime+ '至' + endTime +'的保司案件中，机构内的调查员录入的住宿费金额'
                ths.eq(5).find('.layui-table-cell').attr('title', _title11)

                var _title12 = '打卡时间在'+startTime+ '至' + endTime +'的保司案件中，机构内的调查员录入的跨地市交通费金额'
                ths.eq(6).find('.layui-table-cell').attr('title', _title12)

                var _title13 = '打卡时间在'+startTime+ '至' + endTime +'的保司案件中，机构内的调查员录入的其他费用金额'
                ...
                var _title5 = '保司终审通过时间在'+startTime+ '至' + endTime +'的保司案件中，各机构对应的委托方确认价格之和'
                ...
                var _title3 ='保司终审通过时间在'+startTime+ '至' + endTime +'的保司案件中，各机构的调查费之和'
                ...
                var _title99 = '打卡时间在'+startTime+ '至' + endTime +'的保司案件中，机构内的调查员录入的门诊排查费用金额'
                ths.eq(7).find('.layui-table-cell').attr('title', _title99)

                var _title10 = '打卡时间在'+startTime+ '至' + endTime +'的保司案件中，机构内的调查员录入的体检报告打印费金额'
                ths.eq(8).find('.layui-table-cell').attr('title', _title10)

                var _title11 = '打卡时间在'+startTime+ '至' + endTime +'的保司案件中，机构内的调查员录入的住宿费金额'
                ths.eq(9).find('.layui-table-cell').attr('title', _title11)

                var _title12 = '打卡时间在'+startTime+ '至' + endTime +'的保司案件中，机构内的调查员录入的跨地市交通费金额'
                ths.eq(10).find('.layui-table-cell').attr('title', _title12)

                var _title13 = '打卡时间在'+startTime+ '至' + endTime +'的保司案件中，机构内的调查员录入的其他费用金额'
                ths.eq(11).find('.layui-table-cell').attr('title', _title13)

                var _title14 = '市内交通费+病史费+排查费用+体检报告打印费+住宿费+跨地市交通费+其他费用'
                ths.eq(12).find('.layui-table-cell').attr('title', _title14)
            }
