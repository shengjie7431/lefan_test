...
<div class="main">
    <input type="hidden" name="id" value="${surveyFileCatalog.id}">
    <input type="hidden" name="surveyCode" value="${surveyCode}">
    <div class="title">
        <button class="butList active" onclick="operate('${surveyFileCatalog.id}','${surveyCode}','1000',false);">修改</button>
        <button class="butList defuelt" onclick="operate('${surveyFileCatalog.id}','${surveyCode}','9999',true);">删除</button>
    ...
    <div class="main-boy">
        <div>
            <table class="spec-info" border="0" cellpadding="0" cellspacing="0">
                ...
                <tr>
                    <td>名称</td>
                    <td>${surveyFileCatalog.catalogName}</td>
                    <td>描述</td>
                    <td colspan="3">${surveyFileCatalog.catalogText}</td>
                </tr>
                <tr>
                    <td>创建人</td>
                    <td>${surveyFileCatalog.createByName}</td>
                    <td>创建时间</td>
                    <td><fmt:formatDate value="${surveyFileCatalog.createTime}" pattern="yyyy-MM-dd HH:mm"/></td>
                    <td>更新时间</td>
                    <td><fmt:formatDate value="${surveyFileCatalog.updateTime}" pattern="yyyy-MM-dd HH:mm"/></td>
                </tr>
                </tbody>
            </table>
        </div>
