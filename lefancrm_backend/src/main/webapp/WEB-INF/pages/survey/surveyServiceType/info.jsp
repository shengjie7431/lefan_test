...
<div class="main">
    <input type="hidden" name="id" value="${surveyServiceType.id}">
    <input type="hidden" name="surveyCode" value="${surveyCode}">
    <div class="title">
        <button class="butList active" onclick="operate('${surveyServiceType.id}','${surveyCode}','1000',false);">修改</button>
        <button class="butList defuelt" onclick="operate('${surveyServiceType.id}','${surveyCode}','9999',true);">删除</button>
    ...
    <div class="main-boy">
        <div>
            <table class="spec-info" border="0" cellpadding="0" cellspacing="0">
                ...
                <tr>
                    <td>名称</td>
                    <td>${surveyServiceType.name}</td>
                    <td>描述</td>
                    <td colspan="3">${surveyServiceType.remark}</td>
                </tr>
                <tr>
                    <td>创建人</td>
                    <td>${surveyServiceType.createByName}</td>
                    <td>创建时间</td>
                    <td><fmt:formatDate value="${surveyServiceType.createTime}" pattern="yyyy-MM-dd HH:mm"/></td>
                    <td>更新时间</td>
                    <td><fmt:formatDate value="${surveyServiceType.updateTime}" pattern="yyyy-MM-dd HH:mm"/></td>
                </tr>
                <tr>
                    <td>排序</td>
                    <td>${surveyServiceType.sort}</td>
                </tr>
                </tbody>
            </table>
        </div>
