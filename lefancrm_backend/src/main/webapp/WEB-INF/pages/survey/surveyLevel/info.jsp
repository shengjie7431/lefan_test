...
<div class="main">
    ...
    <div class="title">
        <button class="butList active" onclick="operate('${surveyLevel.id}','${surveyCode}','1000',false);">修改</button>
        <button class="butList defuelt" onclick="operate('${surveyLevel.id}','${surveyCode}','9999',true);">删除</button>
    ...
    <div class="main-boy">
        <div>
            <table class="spec-info" border="0" cellpadding="0" cellspacing="0">
                ...
                <tr>
                    <td>级别名称</td>
                    <td>${surveyLevel.name}</td>
                    <td>级别code</td>
                    <td>${surveyLevel.code}</td>
                    <td>成就点</td>
                    <td>${surveyLevel.successPoint}</td>
                </tr>
                <tr>
                    <td>标的金额</td>
                    <td>${surveyLevel.amount}</td>
                    <td>描述</td>
                    <td colspan="3">${surveyLevel.remark}</td>
                </tr>
                <tr>
                    <td>创建人</td>
                    <td>${surveyLevel.createByName}</td>
                    <td>创建时间</td>
                    <td><fmt:formatDate value="${surveyLevel.createTime}" pattern="yyyy-MM-dd HH:mm"/></td>
                    <td>更新时间</td>
                    <td><fmt:formatDate value="${surveyLevel.updateTime}" pattern="yyyy-MM-dd HH:mm"/></td>
                </tr>
                </tbody>
            </table>
        </div>
