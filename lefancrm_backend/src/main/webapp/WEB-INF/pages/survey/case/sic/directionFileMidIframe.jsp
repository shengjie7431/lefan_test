...
<head>
    ...
    <style>
        ...
        .body-left .files div.file-add.file-add-disabled{
    ...
    <script>
        ...
        function folder(type){
            ...
            $(".files").css("padding-top",height + 20);
            ...
            var folder2 = window.parent.$("#uploadDirectionName").val();//方向名称目录
            var appendPath = "";//子目录
            ...
            var url = "${ctx}/sftp/survey/getFileSftp",param = {"surveyCno":surveyCno,"folder":folder};
            $.ajax({
                url:url,
                data:param,
                success:function(res,param){
                    ...
                    var f = json.success;
                    if (f){
                        var menuCode = window.parent.$("input[name=menuCode]").val();
                        var surveyState = window.parent.$("input[name=surveyState]").val();
                        //费用报销状态
                        // var reState = window.parent.$("input[name=reState]").val();
                        var html ="";
                        // 调查处理页面 survey_state=4 则方向内容不可编辑
                        if (((menuCode == 'dcy-list' && surveyState == 4) || menuCode == 'feeViewSurvey') || ( $("#feeOpr").val() == 'view')) {
                            html="<div class=\"file-add file-add-disabled\">\n";
                            $(".addFile").css({
                                'pointer-events': 'none',
                                'background-color':' #eee'
                            })
                        }else {
                            html="<div class=\"file-add\" onclick=\"$('#materialFileupload').click();\">\n";
                        }
                        $("#materialImgsDiv").html(html+
                            "                <div class=\"icon-add\"></div>\n" +
                            "            </div>\n" +
                            "            <input type=\"hidden\" id=\"fileNum\" value=\"\">\n" +
                            "            <input type=\"hidden\" id=\"img\" name=\"img\" value=\"\" class=\"picToBig\" />\n" +
                            "            <input id=\"materialFileupload\" onclick=\"setDataURL()\" type=\"file\" name=\"file\" multiple style=\"display: none\"/>");
                        ...
                        if(files){
                            for (var i = 0; i < files.length; i++) {
                                ...
                                if (file.folder){//文件
                                    ...
                                    var directionFileMidFlag = $("#directionFileMidFlag").val();
                                    var html="";
                                    if (directionFileMidFlag == 'true'){
                                        var type = "upload";
                                        ...
                                        html="<div ondblclick='folderClick(\"" + file.fileName + "\")' class='file' ><div class='folder-upload' data-file-type='"+type+"'  data-file-path='"+file.filePath+"' ><img src='"+src+"' width='20' height='20' /></div><div class='icon-delete'><a href='javascript:;' onclick='deleteFile(\""+(i + 1)+"\",\""+file.filePath+"\","+null+","+null+")' class='icon-deleteX'>X</a></div><div class='folder-img'></div><a class='img-title' title='" + file.fileName + "'> "+ file.fileName + "(" + (file.fileSize) +")" + "</a></div>";
                                    }else {
                                        html="<div ondblclick='folderClick(\"" + file.fileName + "\")' class='file' ><div class='folder-img'></div><a class='img-title' title='" + file.fileName + "'> "+ file.fileName + "(" + (file.fileSize) +")" + "</a></div>";
                                    }
                                    $("#materialImgsDiv").append(html);
                                }
                            }
                            for (var i = 0; i < files.length; i++) {
                                var file = files[i];
                                ...
                                if (file.folder){//文件
                                    if(file.fileExt == 'rar' || file.fileExt == 'zip'){
                                        $("#materialImgsDiv").append("<div class='file' id='data-id-"+(index_file + 1)+"'  data-id='"+(index_file + 1)+"'><div class='icon-delete'><a href='javascript:;' onclick='deleteFile(\""+(i + 1)+"\",\""+file.filePath+"\","+null+","+null+")' class='icon-deleteX'>X</a></div><img class='image' data-id='"+(index_file + 1)+"' src='${ctx}/img/rar.jpg' /><a class='img-title' title='" + file.fileName + "' href='"+file.filePath+"' target='_blank'> "+ file.fileName +"</a><span title='修改文件名称' class='upd-file-name' data-file-path='"+file.filePath+"' data-file-name='"+file.fileName+"' ><img height='25px' width='25px' src='${ctx}/img/pen.png'></span></div>");
                                    }else if(file.fileExt == 'pdf'){
                                        $("#materialImgsDiv").append("<div class='file' id='data-id-"+(index_file + 1)+"'  data-id='"+(index_file + 1)+"'><div class='icon-delete'><a href='javascript:;' onclick='deleteFile(\""+(i + 1)+"\",\""+file.filePath+"\","+null+","+null+")' class='icon-deleteX'>X</a></div><img class='image' data-id='"+(index_file + 1)+"' src='${ctx}/img/pdf.jpg' onclick='onlinePreview("+file.filePath+")'/><a class='img-title' title='" + file.fileName + "' href='"+file.filePath+"' target='_blank'> "+ file.fileName +"</a><span title='修改文件名称' class='upd-file-name' data-file-path='"+file.filePath+"' data-file-name='"+file.fileName+"' ><img height='25px' width='25px' src='${ctx}/img/pen.png'></span></div>");
                                    }else if(file.fileExt == 'doc' || file.fileExt == 'docx'){
                                        $("#materialImgsDiv").append("<div class='file' id='data-id-"+(index_file + 1)+"'  data-id='"+(index_file + 1)+"'><div class='icon-delete'><a href='javascript:;' onclick='deleteFile(\""+(i + 1)+"\",\""+file.filePath+"\","+null+","+null+")' class='icon-deleteX'>X</a></div><img class='image' data-id='"+(index_file + 1)+"' src='${ctx}/img/word.png' onclick='onlinePreview("+file.filePath+")'/><a class='img-title' title='" + file.fileName + "' href='"+file.filePath+"' target='_blank'> "+ file.fileName +"</a><span title='修改文件名称' class='upd-file-name' data-file-path='"+file.filePath+"' data-file-name='"+file.fileName+"' ><img height='25px' width='25px' src='${ctx}/img/pen.png'></span></div>");
                                    }else if(file.fileExt == 'xls' || file.fileExt == 'xlsx'){
                                        $("#materialImgsDiv").append("<div class='file' id='data-id-"+(index_file + 1)+"'  data-id='"+(index_file + 1)+"'><div class='icon-delete'><a href='javascript:;' onclick='deleteFile(\""+(i + 1)+"\",\""+file.filePath+"\","+null+","+null+")' class='icon-deleteX'>X</a></div><img class='image' data-id='"+(index_file + 1)+"' src='${ctx}/img/excel.png' onclick='onlinePreview("+file.filePath+")'/><a class='img-title' title='" + file.fileName + "' href='"+file.filePath+"' target='_blank'> "+ file.fileName +"</a><span title='修改文件名称' class='upd-file-name' data-file-path='"+file.filePath+"' data-file-name='"+file.fileName+"' ><img height='25px' width='25px' src='${ctx}/img/pen.png'></span></div>");
                                    }else{
                                        var directionFileMidFlag = $("#directionFileMidFlag").val();
                                        var html = "";
                                        if (directionFileMidFlag == "true"){
                                            html="<div class='file' id='data-id-"+(i + 1)+"'  data-id='"+(i + 1)+"'><div class='icon-delete'><a href='javascript:;' onclick='deleteFile(\""+(i + 1)+"\",\""+file.filePath+"\","+null+","+null+")' class='icon-deleteX'>X</a></div><img class='image' data-id='"+(i + 1)+"' src='" + file.filePath + "'/><a class='img-title' title='" + file.fileName + "' href='${ctx}/survey/case/downloadImg?filePath="+file.filePath+"&fileName="+file.fileName+"'> "+ file.fileName +"</a><span title='修改文件名称' class='upd-file-name' data-file-path='"+file.filePath+"' data-file-name='"+file.fileName+"' ><img height='25px' width='25px' src='${ctx}/img/pen.png'></span></div>";
                                        }else {
                                            html = "<div class='file' id='data-id-"+(i + 1)+"'  data-id='"+(i + 1)+"'><img class='image' data-id='"+(i + 1)+"' src='" + file.filePath + "'/><a class='img-title' title='" + file.fileName + "' href='"+file.filePath+"' target='_blank'> "+ file.fileName +"</a><span title='修改文件名称' class='upd-file-name' data-file-path='"+file.filePath+"' data-file-name='"+file.fileName+"' ><img height='25px' width='25px' src='${ctx}/img/pen.png'></span></div>";
                                        }
                                        $("#materialImgsDiv").append(html);
                                    }
                                    index_file ++;
                            ...
                            $("#fileSize").val(index_file);
                            imgInit();//绑定图片的事件
                        }
                    }

                    //2021年9月26日  新需求。第一层只能创建文件夹 第二层只能新增文件
                    if(top == 0){
                        //不可上传附件
                        $(".file-add").hide();
                        $(".addFile").show();
...
<div class="contain">
    ...
    <input type="hidden" name="id" value="${id}" /> <!-- 调查员案件ID-->
    <input type="hidden" name="directionId" value="${directionId}" />
    ...
    <input type="hidden" id="directionFileMidFlag" value="${directionFileMidFlag}">
    <input type="hidden" id="entrustOrgId" value="${entrustOrgId}">
    <div class="body-left" id="body-left">
        ...
        <div class='files' id="materialImgsDiv">
            ...
            <%--            <c:forEach items="${direction.surveyCaseDirectionFiles}" var="file" varStatus="st">--%>
            <%--                <div class="file" data-id="${st.index+1}"  id="data-id-${st.index+1}">--%>
            <%--                    <div class="icon-delete"><a href="javascript:;" onclick="deleteFile(${st.index+1},'${file.commonFile.filePath}',${file.commonFile.id},${direction.id})" class="icon-deleteX">X</a></div>--%>
            <%--                    <img class="image"--%>
            <%--                         <c:if test="${file.fileType==1}">src="${ctx}/img/word.png"</c:if>--%>
            <%--                         <c:if test="${file.fileType==3 || file.fileType==2}">src="${file.commonFile.filePath}"</c:if>--%>
            <%--                         <c:if test="${file.fileType==4}">src="${ctx}/img/excel.png"</c:if>--%>
            <%--                         data-id="${st.index+1}"/>--%>
            <%--                    <a class="img-title" title="${file.commonFile.fileName}">${file.commonFile.fileName}</a>--%>
            <%--                </div>--%>
            <%--            </c:forEach>--%>
    ...
    <div class="body-left body-left-detail">
        <div class="img-detail">
            <div class="img-src" id="pic122"><img src="" class="image" id="pic1"/></div>
            <div class="img-btns">
...
//        }
    ...
    function directionName(){
        var directionName = window.parent.$("#directionName").val();
        $("#directionName").val(directionName);
        var entrustOrgId = $("#entrustOrgId").val();
        var _html =  "<div class='icon-step'  style='width: 30px;display: inline-block; margin-right: 8px;'><div class='index active'>1</div></div>"+"<span>上传附件</span>";
        if (entrustOrgId!=undefined && entrustOrgId != null && entrustOrgId ==155) {//picc
            _html += '<span style="color:red;font-size: 12px;">（此案为PICC案件，图片仅支持jpg格式上传）</span>';
        }
        $("#title").prepend(_html);
        <%--$("#title").append("<div class='icon-step'><div class='index active'>2</div></div>"+"<span>上传" +directionName+"附件 （${direction.surveyCaseDirectionFiles.size()}）</span>");--%>
    }
        ...
        $('#materialFileupload').fileupload({
            ...
            add:function(e,data){
                ...
                if(ext == 'doc'||ext == 'docx'){
                    ...
                    path = "${ctx}/img/pdf.jpg"
                }else{
                    path = file.filePath;
                }
                $("#materialImgsDiv").append("<div class='file' id='data-id-"+indexNum+"'  data-id='"+indexNum+"'><div class='icon-delete'><a href='javascript:;' onclick='deleteFile(\""+indexNum+"\",\""+file.filePath+"\","+null+","+null+")' class='icon-deleteX'>X</a></div><img class='image' data-id='"+indexNum+"' src='" + path + "'/><a class='img-title' title='" + file.fileName + "' href='${ctx}/survey/case/downloadImg?filePath="+file.filePath+"&fileName="+file.fileName+"'> "+ file.fileName +"</a></div>");
//                $("#materialImgsDiv").append("<div class='file' data-id='"+indexNum+"'><div class='icon-delete'>X</div><img class='image' data-id='"+indexNum+"' src='" + file.filePath + "'/></div>");
    ...
    //上传中的图片删除
    function deleteFile(indexNum,filePath,fileId,directionId) {
        var url = "${ctx}/survey/case/sic/operate";
        var param = {"fileId":fileId,"filePath" : filePath,"btnCode":"deleteFile","directionId":directionId};
        if(confirm('是否确认？')){
            ajaxSubmit1(url,param,function(v,e,p){
                //删除页面上的图片
                folder("");
            });
        }
    ...
    function photoSee1() {
        ...
        $('.rolateleft').on('click', function () {
            ...
            console.log('r-left----1-------', imgStyle)
            AutoSize(url)
            console.log('r-left----2-------', imgStyle)
        ...
        function AutoSize(src) {
            ...
            imgStyle = $('#pic1').attr('style')
            imgWidth = $('#pic1').width()
            imgHeight = $('#pic1').height()
            console.log('bbbbb',imgStyle, imgWidth, imgHeight)
        }

        $('.img-src').on('contextmenu', function () { // 阻止右键菜单弹出
            return false;
        })
        ...
        (function () {
            ...
            addEvent(oImg, 'mousedown', function (ev) {
                if (ev.which === 3) {
                    $('.img-src img').attr('style', imgStyle)
                }
                var oEvent = prEvent(ev),
                    ...
                    startMove = function (ev) {
                        ...
                        oImg.style.marginLeft = L + 'px';
                        oImg.style.marginTop = T + 'px';
            ...
            addWheelEvent(oImg, function (delta) {
                ...
                if (w_copy < imgWidth || h_copy < imgHeight) {
                    oImgCopy.attr('style', imgStyle)
                }
            });
        })()
