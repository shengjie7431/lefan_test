...
<%--                </div>--%>
        ...
        <div class='files' id="materialImgsDiv">
            <c:forEach items="${direction.surveyReimbursementFileDtoList}" var="item" varStatus="status">
                <div class='file'  id='data-id-${item.id}' data-value="${status.index}"  data-id='0'><div class='icon-delete' ${isdelete=='no'?'hidden':''}><a href='javascript:;' onclick='deleteFile(${item.id},null,${item.id},null)' class='icon-deleteX'>X</a></div><img class='image' data-id='${status.index+1}' src='${item.fileUrl}'/><a class='img-title'  target='_blank'></a></div>
            </c:forEach>
        </div>
    </div>
...
<script type="text/javascript">
    ...
    var result = getFiles(dataId);
    var files =result[0];
    if (files!=null && files!=""){
        files = files.split(",");
        for (let i = 0; i <files.length ; i++) {
            var str = "<div class='file'  data-index='"+i+"'  data-id='0'><div class='icon-delete'><a href='javascript:;' onclick='deleteOne("+dataId+",this)' class='icon-deleteX'>X</a></div><img class='image' data-id='"+i+"' src='"+files[i]+"'/><a class='img-title'  target='_blank'></a></div>";
    ...
    function  getFiles(dataId) {
        var result = [];
        if (dataId == 1){
            result[0] = sessionStorage.getItem("medicalHistoryFiles");
            result[1] = "medicalHistoryFiles";
        }else if (dataId==2){
            result[0] = sessionStorage.getItem("troubleshootingFiles");
            result[1] = "troubleshootingFiles";
        }else if (dataId==3){
            result[0] = sessionStorage.getItem("printingFiles");
            result[1] = "printingFiles";
        }else if (dataId==4){
            result[0] = sessionStorage.getItem("accommodatioFiles");
            result[1] = "accommodatioFiles";
        }else if (dataId==5){
            result[0] = sessionStorage.getItem("carFiles");
            result[1] = "carFiles";
        }else if (dataId==6){
            result[0] = sessionStorage.getItem("trainFiles");
            result[1] = "trainFiles";
        }else if (dataId==7){
            result[0] = sessionStorage.getItem("airFiles");
            result[1] = "airFiles";
        }else if (dataId==8){
            result[0] = sessionStorage.getItem("selfDrivingFiles");
            result[1] = "selfDrivingFiles";
        }else if (dataId==9){
            result[0] = sessionStorage.getItem("otherMoneyFiles");
            result[1] = "otherMoneyFiles";
        }else if (dataId==10){
            result[0] = sessionStorage.getItem("innerCityFiles");
            result[1] = "innerCityFiles";
        }
        return result;
    }

    //未保存的删除
    function deleteOne(dataId,_this){
        var result = getFiles(dataId);
        var files = result[0];
        if (files!=null){
            if (confirm("是否确认？")){
                ...
                //改变父页面的值
                var old = window.parent.$('.v-p-files[data-id='+ dataId +']').attr("data-value");
                var newValue = old - 1;
                window.parent.$('.v-p-files[data-id='+ dataId +']').text("凭证("+newValue+")");
                window.parent.$('.v-p-files[data-id='+ dataId +']').attr("data-value",newValue);
                if (files.length>0){
                    sessionStorage.setItem(result[1],files.toString());
                    window.parent.$("input[name="+result[1]+"]").val(files.toString());
                }else {
                    sessionStorage.removeItem(result[1]);
                    window.parent.$("input[name="+result[1]+"]").val("");
                }
            }
        }
    }
...
//                $("#materialImgsDiv").append("<div class='file' data-id='"+indexNum+"'><div class='icon-delete'>X</div><img class='image' data-id='"+indexNum+"' src='" + file.filePath + "'/></div>");
    ...
    function deleteFile(indexNum,filePath,fileId,directionId) {
        ...
        var param = {"fileId":fileId,"filePath" : filePath,"btnCode":"deleteReimFile","directionId":directionId};
        if(confirm('是否确认？')){
            ajaxSubmit1(url,param,function(v,e,p){
                //删除页面上的图片
                $("#data-id-" + indexNum).remove();
                $(this).parent().remove();
                //更新values数据
                var newList = [];
                for (var i = 0; i < fileList.length; i++) {
                ...
                fileList = newList;
                $("#fileList").val(JSON.stringify(newList));
                ...
                if (newList.length>0){
                    // sessionStorage.setItem(result[1],files.toString());
                    window.parent.$("input[name="+result[1]+"]").val(newList.toString());
                }else {
                    sessionStorage.removeItem(result[1]);
                }
            });
