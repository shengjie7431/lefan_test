    ...
    <div class="detail_context_pic">
        ...
        <div class="detail_context_pic_bot">
            ...
            <div class="detail_picbot_mid">
                <ul>
                    <c:forEach items="${surveyCaseFiles}" var="file">
                        <li>
                            <a href='javascript:void(0);'>
                                <img width='90px' height='60px'
                                     bigimg='
                                        <c:if test="${file.fileType == 1}">${ctx}/img/word.png</c:if>
                                        <c:if test="${file.fileType == 2}">${file.commonFile.filePath}</c:if>
                                        <c:if test="${file.fileType == 3}">${ctx}/img/rar.jpg</c:if>
                                        <c:if test="${file.fileType == 4}">${ctx}/img/excel.png</c:if>
                                        <c:if test="${file.fileType == 5}">${ctx}/img/pdf.jpg</c:if>
                                        <c:if test="${file.fileType == 6}">${ctx}/img/txt.png</c:if>
                                     '
                                 <c:if test="${file.fileType == 1}">src='${ctx}/img/word.png'</c:if>
                                 <c:if test="${file.fileType == 2}">src='${file.commonFile.filePath}'</c:if>
                                 <c:if test="${file.fileType == 3}">src='${ctx}/img/rar.jpg'</c:if>
                                 <c:if test="${file.fileType == 4}">src='${ctx}/img/excel.png'</c:if>
                                 <c:if test="${file.fileType == 5}">src='${ctx}/img/pdf.jpg'</c:if>
                                 <c:if test="${file.fileType == 6}">src='${ctx}/img/txt.png'</c:if>
                                />
                            </a>
                        </li>
                    </c:forEach>
