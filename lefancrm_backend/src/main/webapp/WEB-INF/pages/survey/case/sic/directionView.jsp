...
<div class="contain">
    ...
    <div class="body-right">
        <form id="editForm" role="form" action="${ctx}/survey/case/sic/operate" method="post">
            ...
            <input type="hidden" id="isUserLetterInfo" name="isUserLetterInfo" value="${dto.isUserLetterInfo}"/>
            ...
            <input type="hidden" id="fristTaskId" value="${fristTaskId}"/>
            <input type="hidden" id="fristTaskName" value="${fristTaskName}"/>
        ...
        <div class="form">
            ...
            <div class="cell">
                ...
                <div class="value">
                    <input type="text" id="directionName" name="directionName" class=""  onblur="findName(this)" />
                    <%--验证不重复的方向名称--%>
                    <input type="hidden" id="uploadDirectionName" name="uploadDirectionName" class=""  value="${directionName}" />
                </div>
            </div>
            <div class="cell">
                ...
                <div class="value" data-id="address">
                    ...
                    <input type="text" class=""  autocomplete="off" placeholder="请输入区县名或城市名查询"  id="areaName" name="areaName" required="required"
                           <c:if test="${firstHistory!=null}">value="${firstHistory.district} ${firstHistory.city} ${firstHistory.province}"</c:if>
                           onfocus="selectByAreaName()">
                    <%--onblur="selectByAreaName()"光标离开触发--%>
                ...
                <%--<div class="value">--%>
                    <%--<select name="areaId" id="areaId" class="singleSelect form-control" onchange="changeArea1()">--%>
                        ...
                        <%--<c:forEach items="${directionAreaHistory}" var="item">--%>
                            <%--<option value="${item.districtId},${item.cityId},${item.provinceId},${item.district},${item.city},${item.province},${item.cityType},${item.areaType},${item.cityTypeName}" >${item.district}${item.city}${item.province}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style="float:right;">${item.cityTypeName}</span></option>--%>
                        <%--</c:forEach>--%>

                        <%--<c:forEach items="${commonAreaDtos}" var="item">--%>
                            <%--<option <c:if test="${areaId == item.areaId}">selected="selected" </c:if> value="${item.areaId},${item.areaId2},${item.areaId3},${item.areaName1},${item.areaName2},${item.areaName3},${item.cityType},${item.areaType},${item.cityTypeName}" >${item.areaName1}${item.areaName2}${item.areaName3}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style="float:right;">${item.cityTypeName}</span></option>--%>
                        <%--</c:forEach>--%>
                    <%--</select>--%>
                <%--</div>--%>
                <input type="hidden" id="province" name="province" value="${firstHistory.province}">
                <input type="hidden" id="city" name="city" value="${firstHistory.city}">
                <input type="hidden" id="district" name="district" value="${firstHistory.district}">
                <input type="hidden" id="provinceId" name="provinceId" value="${firstHistory.provinceId}">
                <input type="hidden" id="cityId" name="cityId" value="${firstHistory.cityId}">
                <input type="hidden" id="districtId" name="districtId" value="${firstHistory.districtId}">
                <input type="hidden" id="areaType" name="areaType" value="${firstHistory.areaType}">
                <input type="hidden" id="regionType" name="regionType" value="${firstHistory.regionType}">
                <input type="hidden" id="directionId" name="directionId" value="">
                ...
                <div class="value">
                    <div class="btns">
                        <c:forEach items="${dto.tasks}" var="item">
                            <div style="cursor:pointer" title="${item.taskRemark}"
                                 <%--<c:if test="${item.taskId == fristTaskId}">class="item active" </c:if>--%>
                                 <%--<c:if test="${item.taskId != fristTaskId}">class="item" </c:if>--%>
                                    class="item"
                                 onclick="initNewName(${item.taskId},'${item.taskName}')" id="task_${item.taskId}" data-value="${item.taskId}"  name="taskIds" >
                                    ${item.taskName}
                            </div>
                        </c:forEach>
                    </div>
                    <input type="hidden" value="" id="taskId" name="taskId" required="required"/>
                    <input type="hidden" value="" id="curTaskId" required="required"/>
                    <input type="hidden" value="" id="taskName" name="taskName" required="required"/>
                </div>
            </div>
            <div class="cell pd0" id="taskInfoContent" style="display: none" >
                <div class="label">任务子类</div>
                <div class="value">
                    <div class="btns" id="newIdList">
                        <input type="hidden" value="" id="newId" name="newId" required="required"/>
                    </div>

                </div>
            </div>
            <div class="cell" style="display: none" id="directionModel">
                <div class="label">任务结果</div>
                <div class="value">
                    <div class="btns" id="directionResultTypeIdList">
                        <%--<div class="item active">有结果</div>--%>
                            <input type="hidden" value="" id="directionResultTypeId" name="directionResultTypeId"/>
                    </div>
                </div>
            </div>
            ...
            <div class="cell">
                ...
                <div class="value">
                    <input type="text" id="surveyReason" name="surveyReason" placeholder="必填，且不可和方向名称重复。如“华东医院病史2份”" class="" autocomplete="off" value="${direction.surveyReason}" />
                </div>
            </div>
            <div class="cell">
                <div class="label">材料原件</div>
                <div class="value">
                    <input type="hidden" name="materRaw"/>
                    <div class="label-bars materRaw"  id="materRaw" data-value="">
                        <div class="label-bar" data-id="1">有</div>
                        <div class="label-bar" data-id="0">无</div>
                    </div>
                </div>
            </div>
            <div class="cell">
            ...
            <div class="cell">
                ...
                <div class="value">
                    ...
                    <div class="label-bars isSun"  id="isSun" data-value="0">
                        <div class="label-bar active" data-id="0">否</div>
                        <div class="label-bar" data-id="1">是</div>
                    </div>
                </div>
            </div>
        ...
        <div class="form-btns">
                ...
                <%--<button type="submit" name="btnUpload" onclick="return validFile('${btnCode}');" class="btn btn-success loading-btn" data-loading-text="Loading..." autocomplete="off"><span class="glyphicon glyphicon-ok"></span>保存并关闭</button>&nbsp&nbsp&nbsp--%>
...
function load(){
    ...
    function validFile(btnCode){
        ...
        ajaxSubmit(url,param,function(v,e,p){
            if(e.data.code==='0000'){
                ...
                if(materRaw == '' || materRaw == null){
                    alert("材料原件信息必选！");return false;
                }
                ...
                }else if(btnCode == 'direction' || btnCode == 'successDirection' || btnCode == 'successDirectionPopup' || btnCode == 'directionAgain')
                {
                    ...
                    var directionText = $("#directionText").val();
                    if(!directionText){
                        i = 0;
                        alert("请填写方向内容");return false;
                    }
                    var directionResultTypeId = $("#directionResultTypeId").val();
                    if(!directionResultTypeId){
                        i = 0;
                    ...
                    if(btnCode == 'successDirection'){ // “已完成全部方向” 的弹窗中的“确定提交”按钮，
                        two = 1; //two = 1的意思是直接刷新父级，不需要--confirm('是否确认？')
                        /*var isSun = $("#isSun").val();
                        if(!isSun){
                            i = 0;
   ...
   function textSelect(textBox,start,end){
    ...
    function showDirectionView() {
        var isUserLetterInfo = $("#isUserLetterInfo").val(); //是否使用了介绍信
        if (isUserLetterInfo == 'false') {
            var dialog = $(".dalogs2");
            dialog.show();
//            var position = $("#successDirectionBtn").offset();//以按钮为定位
...
//            });
            ...
            html += '<div class="title">选择使用介绍信情况</div>';
            html += '<div class="content">';
            html += '<div class="btn-area">';
/*            html += '<div class="c-btn" onclick="showDirectionView2(1);">使用</div>';
            html += '<div class="c-btn2" onclick="showDirectionView2(0);">未使用</button>';*/

            html += '<div class="c-btn isUse" onclick="addDirectionSuccess(1);">使用</div>';
            html += '<div class="c-btn2 isUse" onclick="addDirectionSuccess(0);">未使用</button>';
...
//            showDirectionView2('');
    ...
    function addDirectionSuccess(isUser){
        var _this = $('.isUse')
...
//        }
    ...
    function showDirectionView2(isUser){
        var dialog2 = $(".dalogs2");
        dialog2.hide();
        i = 0; // 控制多次提交 ： 在弹窗前，走了一遍validFile。所以再次还原i的值，再次控制
        var dialog = $(".dalogs");
        if(isUser!=null || isUser!=""){
            dialog.hide();
            $("#isUser").val(isUser);
...
//        html += '<div class="c-btn" onclick="javascript:hidDirectionView();">取消</div>';
        html += '<button type="button" name="add-direction-success" class="btn btn-success loading-btn" onclick="return addDirectionSuccess(\'successDirection\');">确定提交</button>';
        html += '</div></div></div></div>';
        $(".d_content").html(html);
    }
...
//        $("[name=btnUpload]").attr("disabled","true");
    ...
    var fristTaskName = '${fristTaskName}'; //案件包含的任务类型第一个
    var fristTaskId = '${fristTaskId}';
    // initNewName(fristTaskId,fristTaskName);//初始化，默认选中“仅医疗排查（含门诊或住院）”
    //点击任务类型：加载任务子类
    function initNewName(id,name){
   ...
   function findName(cur){
       var directionName = $("#directionName").val()
       var newId = $("#newId").val();
       if(newId == 315 || newId == 327){
           var html ="";
           html += '调查人员至'+directionName+'病案室，调阅被保人住院号的住院病史。经核实病史首页患者信息与被保人一致。\n'
           html += '出入院时间：\n';
           html += '入院诊断：\n';
           html += '出院诊断：\n';
           html += '主诉：\n';
    ...
    function choiceNewIds(id) {
        ...
        ajaxSubmit("${ctx}/baseSurvey/selectInfoByRelationId",{"surveyCode":"taskInfoContent","btnCode":"1000","taskInfoContentId":id,"id":cruxId},function(v,e,p){
            ...
            if(e.data.results.length > 1){
                ...
                $("#directionModel").hide();
                ajaxTaskSubSorce(id,e.data.results[0].directionResultTypeId);
            }else{
                $("#directionModel").hide();
            }
            for(var i = 0; i < e.data.results.length; i++){
                var val = e.data.results[i];
                $("#directionResultTypeIdList").append("<div class='item' style='cursor:pointer' onclick='choiceResult("+id+","+val.directionResultTypeId+",\""+val.directionResultTypeCode+"\")' id='directionResultTypeId_"+val.directionResultTypeId+"' name='directionResultTypeIds' value='" + val.directionResultTypeId + "'>" + val.directionResultTypeName +"</div>");
            }
        });

        if(id == 315 || id == 327){
            var html ="";
            html += '调查人员至'+ $("#directionName").val() +'病案室，调阅被保人住院号的住院病史。经核实病史首页患者信息与被保人一致。\n'
            html += '出入院时间：\n';
            html += '入院诊断：\n';
            html += '出院诊断：\n';
            html += '主诉：\n';
   ...
   function ajaxTaskSubSorce(newId,directionResultTypeId){
    ...
    function choiceResult(newId,directionResultTypeId,code) {//任务子类id，方向结果类型id
        ...
        if((code =="medical"||code =="medical01"||code =="medical02"||code =="medical03") && (newId == 301 || newId == 303 || newId == 300 || newId == 319)){
            ...
            html += '放射：';
            $("#directionText").val(html);
            $("#medicalNumber").val($("#text_box").val());
        }else{
            $("#directionText").val(null);
            $("#medicalNumber").val(null);
            $("#medical").hide();
