        ...
        <table class="table table-hover">
            ...
            <c:forEach items="${apiRsp.results}" var="item">
                <tr>
                    ...
                    <c:if test="${caseCode == 'org'}">
                        <td>
                            <c:if test="${item.surveyItem == null}">
                                <a data-value="${item.surveyItem}" title="暂无数据" class="viewdesc">查看描述</a>
                            </c:if>
                            <c:if test="${item.surveyItem !=null}">
                                <a data-value="${item.surveyItem}" title="${item.surveyItem}" class="viewdesc">查看描述</a>
                            </c:if>
                        </td>
                        <td width="100">
                            <c:if test="${item.orgCaseState == 11 || item.orgCaseState == 12}">
                                单点
                            </c:if>
                            <c:if test="${item.orgCaseState == 13}">
                                全案
                            </c:if>
                        </td>
<%--                        <td width="100">${item.orgCaseState}</td>--%>
                        <td width="100"><fmt:formatDate value="${item.orgCreateTime}" pattern="yyyy-MM-dd"/></td>
                        ...
                        <c:if test="${item.orgAgingReal==0 && item.orgAgingOver==0 }">
                            <td width="100">${item.orgAgingDay}天</td>
                        </c:if>
                        <c:if test="${item.orgAgingOver>0}">
                            <td width="100" class="color1">${item.orgAgingReal}天</td>
                        </c:if>
                        <c:if test="${item.orgAgingReal > 0 && item.orgAgingOver==0}">
                            <td width="100" class="color2">${item.orgAgingReal}天</td>
                        </c:if>
                    ...
                    <c:if test="${caseCode == 'user'}">
                        <td>
                            <a data-value="${item.taskName}" title="${item.taskName}"
                        ...
                        <td width="100"><fmt:formatDate value="${item.invAssignDate}" pattern="yyyy-MM-dd"/></td>
                        <td width="100"><fmt:formatDate value="${item.invCReportDate}" pattern="yyyy-MM-dd"/></td>
                        <td width="100"><fmt:formatDate value="${item.surveyEndTime}" pattern="yyyy-MM-dd"/></td>
                        <c:if test="${item.surveyAgingOver == 0}">
                            <td width="100" >${item.surveyAgingReal}天</td>
                        </c:if>
                        <c:if test="${item.surveyAgingOver>0}">
                            <td width="100" class="color1">${item.surveyAgingReal}天</td>
                        </c:if>
                        <c:if test="${item.surveyAgingOver>0 && item.surveyAgingOver == 0}">
                            <td width="100" class="color2">${item.surveyAgingOver}天</td>
                        </c:if>
                        <c:if test="${item.surveyReturnState>0}">
                            <td width="100">是</td>
                        </c:if>
                        <c:if test="${item.surveyReturnState==0}">
                            <td width="100">否</td>
                    ...
                    <c:if test="${item.isSun == null || item.isSun == 0}">
                        <td width="100">否</td>
                    </c:if>
                    ...
                    <td>
                        <a href="javascript:void(0);" onclick="info(${item.surveyInfoId},${item.assignCaseId})" class="aColor">详情</a>
                    </td>
                </tr>
            </c:forEach>
            </tbody>
