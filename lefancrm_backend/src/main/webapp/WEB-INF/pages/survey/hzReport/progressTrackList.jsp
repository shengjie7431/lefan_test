...
//}
    ...
    var _cols1 = [
        [{
            ...
            children: function (row) {
                if (row.orgType == 1) {
                    Object.assign(paramsSubmit, {son: row.orgType, menuCode: 'progressTrack-son',orgId: row.surveyOrgId,surveyOrgIds:''})
                    return [{
                        title: function (row) {
                        ...
                        even: true,cols: [
                            [{
                                ...
                                children: function (row) {
                                    Object.assign(paramsSubmit, {son: row.orgType, orgId: row.surveyOrgId, menuCode: 'progressTrack-son-item'})
                                    return [{
                                        title: function (row) {
                    ...
                    Object.assign(paramsSubmit, {son: row.orgType, surveyOrgId: row.surveyOrgId, menuCode: 'progressTrack-son-item',orgId: row.surveyOrgId})
                    return [{
                        title: function (row) {
                            first = row.surveyOrgName
    ...
    function showTitle(thsJ, child, iter) {
      ...
      if (!child){
          var _title3 = '委托时间在'+startTime+ '至' + endTime +'的互助案件中，分派给机构的案件数量'
          ...
          var _title5 = '委托时间在'+startTime+ '至' + endTime +'的互助案件中，复审通过的案件数量'
          ...
          var _title7 ='委托时间在'+startTime+ '至' + endTime +'的互助案件中，还在机构手里的案件（包括调查中，初审中）'
          ths.eq(4).find('.layui-table-cell').attr('title', _title7)

          var _title8 = '委托时间在'+startTime+ '至' + endTime +'的互助案件中，还在机构手里且机构截止日期大于等于0天，小于等于3天的案件'
          ths.eq(5).find('.layui-table-cell').attr('title', _title8)

          var _title9 = '委托时间在'+startTime+ '至' + endTime +'的互助案件中，还在机构手里且机构截止日期小于0天的案件'
          ths.eq(6).find('.layui-table-cell').attr('title', _title9)

          var _title10 = '委托时间在'+startTime+ '至' + endTime +'的互助案件中，还在机构手里且机构截止日期大于等于10天的案件'
          ths.eq(7).find('.layui-table-cell').attr('title', _title10)

          var _title11 = '委托时间在'+startTime+ '至' + endTime +'的互助案件中，已经复审通过且该机构下的调查员拿到了阳性积分的案件数量（不是最早发现阳性的不算）'
          ths.eq(8).find('.layui-table-cell').attr('title', _title11)
      }else {
          var _title3 = '委托时间在'+startTime+ '至' + endTime +'的互助案件中，分派给调查员的案件数量'
          ths.eq(0).find('.layui-table-cell').attr('title', _title3)
          ...
          var _title7 ='委托时间在'+startTime+ '至' + endTime +'的互助案件中，还在调查员手里的案件（包括调查中，初审中）'
          ths.eq(4).find('.layui-table-cell').attr('title', _title7)

          var _title8 = '委托时间在'+startTime+ '至' + endTime +'的互助案件中，还在调查员手里，调查员截止日期小于等于3天且未超期的案件'
          ths.eq(5).find('.layui-table-cell').attr('title', _title8)

          var _title9 = '委托时间在'+startTime+ '至' + endTime +'的互助案件中，还在调查员手里且调查员截止日期小于0天的案件'
          ths.eq(6).find('.layui-table-cell').attr('title', _title9)

          var _title10 = '委托时间在'+startTime+ '至' + endTime +'的互助案件中，还在调查员手里且调查员截止日期大于等于10天的案件'
          ths.eq(7).find('.layui-table-cell').attr('title', _title10)

          var _title11 = '委托时间在'+startTime+ '至' + endTime +'的互助案件中，已经复审通过且该调查员拿到了阳性积分的案件数量（不是最早发现阳性的不算）'
          ths.eq(8).find('.layui-table-cell').attr('title', _title11)
      }
    }
...
function showCaseList (data,caseCode,colType){
    ...
    var caseState = data.caseState;
    var orgCaseState = data.orgCaseState;
    ...
    openDialog({
        ...
        height:height,
        width:width,
        url:"${ctx}/survey/hzReport/popup?menuCode=progressTrack&btnCode=case&caseCode=" + caseCode+ "&caseState="+caseState+"&entrustOrgIds="+entrustOrgIds
                + "&orgCaseState="+orgCaseState+ "&surveyState="+surveyState+ "&surveyOrgId="+surveyOrgId+ "&surveyUserId="+surveyUserId
                + "&startTime="+startTime+ "&endTime="+endTime+ "&searchType="+searchType+"&scoreRole="+scoreRole+"&colType="+colType+"&orgLevel="+orgLevel
    });
}
</script>
