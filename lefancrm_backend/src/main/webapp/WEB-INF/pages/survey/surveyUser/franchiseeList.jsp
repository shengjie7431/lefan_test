...
<div class="main administrator">
    ...
    <div class="panel-heading">
        <div class="pin">
            <form class="form-inline" role="form" action="${ctx}/baseSurvey/popup?surveyCode=${surveyCode}&userId=${userId}&btnCode=${btnCode}" method="post"  onsubmit="return false;">
                <div class="form-group">
                    调查方机构:<input name="name" type="text" value="${name}" class="form-control">
                </div>
                <div class="btn-group">
                ...
                <tr>
                    <td>
                        <input type="checkbox" name="buss" value="${item.id}"
                            <c:forEach items="${userFranchisees}" var="my">
                                <c:if test="${my.franchiseeId == item.id }">
                                    checked="checked"
                                </c:if>
                            </c:forEach>
                        />
                    ...
                    <td class="companyName">
                        ${item.name}
                            <c:forEach items="${userFranchiseeAll}" var="all">
                                <c:if test="${all.franchiseeId == item.id }">
                                    <span style="color: #b3aaaa">(${all.userName})</span>
                                </c:if>
                            </c:forEach>
                    </td>
                </tr>
