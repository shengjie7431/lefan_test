...
<div class="main">
    <input type="hidden" name="id" value="${surveyLfcoinRule.id}">
    <input type="hidden" name="surveyCode" value="${surveyCode}">
    <div class="title">
        <button class="butList active" onclick="operate('${surveyLfcoinRule.id}','${surveyCode}','1000',false);">修改</button>
        <button class="butList defuelt" onclick="operate('${surveyLfcoinRule.id}','${surveyCode}','9999',true);">删除</button>
    ...
    <div class="main-boy">
        <div>
            <table class="spec-info" border="0" cellpadding="0" cellspacing="0">
                ...
                <tr>
                    <td>规则code</td>
                    <td>${surveyLfcoinRule.code}</td>
                    <td>规则说明</td>
                    <td>${surveyLfcoinRule.remark}</td>
                    <td>乐凡币数量</td>
                    <td>${surveyLfcoinRule.num}</td>
                </tr>
                <tr>
                    <td>创建人</td>
                    <td>${surveyLfcoinRule.createByName}</td>
                    <td>创建时间</td>
                    <td><fmt:formatDate value="${surveyLfcoinRule.createTime}" pattern="yyyy-MM-dd HH:mm"/></td>
                    <td>更新时间</td>
                    <td><fmt:formatDate value="${surveyLfcoinRule.updateTime}" pattern="yyyy-MM-dd HH:mm"/></td>
                </tr>
                </tbody>
            </table>
        </div>
