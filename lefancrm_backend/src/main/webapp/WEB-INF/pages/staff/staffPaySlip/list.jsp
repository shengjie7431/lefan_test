...
<div class="main administrator">
    <div class="main-top">
        <h3>工资条管理列表 <small>共<span>${apiRsp.count}</span>个</small></h3>
    </div><!--main-top-->
        ...
        <div class="table-content">
            <table class="table table-hover">
                ...
                <tr>
                    <th width="100">月份</th>
                    <th width="100">状态</th>
                    <c:if test="${(type=='info'&& !organManager) || showPassWord || isHr}">
                ...
                <c:forEach items="${apiRsp.results}" var="item">
                    <tr>
                        ...
                        <td>
                            ...
                            <c:if test="${type=='view' }">
                                <a href="javascript:info('${item.id}','${item.workTime}','view');">查看</a>
                            </c:if>
                            <c:if test="${type=='info' }">
                                <c:if test="${organManager}">
                                    <c:if test="${hrManager || isHr || superManager}">
                                        <a href="javascript:info('${item.id}','${item.workTime}');">处理</a>&nbsp;&nbsp;
                                    </c:if>
                                    <c:if test="${!hrManager && !isHr && !superManager}">
                                        <a href="javascript:info('${item.id}','${item.workTime}','organManager');">处理</a>&nbsp;&nbsp;
                                    </c:if>
                                </c:if>
                                <c:if test="${!organManager}">
                                    <a href="javascript:info('${item.id}','${item.workTime}');">处理</a>&nbsp;&nbsp;
                                </c:if>
                                <c:if test="${item.slipState == 0}">
                                    <a href="javascript:operate('${item.id}','9999',true);">删除</a>
                                </c:if>
                                <c:if test="${item.slipState == 5 && (hrManager || isHr)}">
                                    <c:if test="${item.isSend == 0 || item.isSend == null}"><a href="javascript:operate('${item.id}','sendWechat',true);">发送工资条</a></c:if>
                                    <c:if test="${item.isSend == 1}">已发送工资条</c:if>
                                </c:if>
                            </c:if>
                        </td>
                    </tr>
...
<script>
    ...
    function operate(id,btnCode,ajax){
        if(ajax){
            var url = "${ctx}/staff/operate",param = {"id":id,"btnCode":btnCode,"operateCode":'slip'};
            if(confirm('是否确认？')){
