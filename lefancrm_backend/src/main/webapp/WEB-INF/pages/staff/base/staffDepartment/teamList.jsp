...
<div class="main administrator">
    ...
    <div class="panel panel-info">
        <form id="editForm" role="form" action="${ctx}/staff/update" method="post">
            ...
            <input type="hidden" name="surveyCode" value="departmentTeam">
            ...
            <table class="table table-hover">
                            ...
                            <tr>
                                <td>
                                    <input type="checkbox" name="buss" value="${item.id}"
                                        <c:forEach items="${departmentTeams}" var="my">
                                            <c:if test="${my.teamId == item.id }">checked="checked"</c:if>
                                        </c:forEach>/>
                                </td>
                                <td>${item.name}</td>
                            </tr>
                ...
                <c:if test="${btnCode ==2000}">
                    ...
                    <tbody class="class-list">
                    <c:forEach items="${departmentTeams}" var="item">
                        <tr>
                            <td>
                                ${item.teamName}
                            </td>
                            <td>
                                <a href="javascript:fitJobPost('${item.teamId}','team','1000','设置岗位');">设置岗位</a>
                                <a href="javascript:fitJobPost('${item.teamId}','team','2000','名下岗位');">名下岗位</a>
                            </td>
                        </tr>
                    </c:forEach>
                    </tbody>
...
<script type="text/javascript">
    ...
    var fitJobPost = function(teamId,surveyCode,btnCode,title){
        ...
        openDialog({
            ...
            height:400,
            width:1100,
            url:"${ctx}/staff/popup?companyId="+companyId+"&organId="+organId+"&departmentId="+departmentId+"&surveyCode="+surveyCode+"&btnCode="+btnCode+"&teamId="+teamId,
            load:true
        });
    }
</script>
