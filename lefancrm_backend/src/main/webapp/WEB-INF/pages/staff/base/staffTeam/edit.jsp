...
<div class="container">
    <form id="editForm" role="form" action="${ctx}/staff/update" method="post">
        ...
        <input type="hidden" name="id" id="id" value="${staffTeam.id}">
        <div class="form-group">
            <table class="table">
                ...
                <tr>
                    ...
                    <td width="70%">
                        <input type="text" id = "name" name="name" value="${staffTeam.name}" required="required" onblur="selectName('${staffTeam.id}',2000,this)" class="form-control">
                    </td>
                </tr>
                <tr>
                    ...
                    <td width="70%">
                        ...
                        <select class="form-control" required="required" name="state" id="state">
                            <option value="0" <c:if test="${staffTeam.state == 0}"> selected="selected" </c:if>>启用</option>
                            <option value="1" <c:if test="${staffTeam.state == 1}"> selected="selected" </c:if>>停用</option>
                        </select>
                    </td>
                </tr>
                </tbody>
...
<script type="text/javascript">
    ...
    function selectName(teamId,btnCode,obj){
        var result = obj.value;
        if(result ==null || result==""){
            return;
        }
        ajaxSubmit("${ctx}/staff/selectStaffInfoByRelationId",{"teamId":teamId,"surveyCode":'${surveyCode}',"btnCode":btnCode,"name":result},function(v,e,p){
