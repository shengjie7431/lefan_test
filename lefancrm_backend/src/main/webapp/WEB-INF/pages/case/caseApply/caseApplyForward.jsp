    ...
    <form id="editForm" role="form" action="${ctx}/caseApply/forwardSubmit" method="post">
        ...
        <div class="form-group">
            <table class="table">
                ...
                <tr>
                    <th width="20%" class="active">机构</th>
                    <td width="80%">
                        <select id = "orgId" name="orgId" required="required"  style="width: 170px;"  onclick="onChangeOrgId()" class="form-control">
                            <option value="0">请选择</option>
                            <c:forEach items="${apiRsp.results}" var="item1">
                ...
                <tr>
                    <th width="20%" class="active">业务员</th>
                    <td width="80%">
                        <select id = "loanApplicationUserId" name="loanApplicationUserId" required="required"  style="width: 170px;" class="form-control">
                            <option value="0">请选择</option>
                        </select>
                    </td>
                </tr>
...
//        $("#content").text(editor1.html());
    ...
    function onChangeOrgId(){
        var orgId = $("#orgId").val();
        if(orgId == 0){
            return;
        }
        //查询机构下的业务员
        ajaxSubmit("${ctx}/caseApply/selectSalesUserByOrgId",{"orgId":orgId},function(v,e,p){
            $("#loanApplicationUserId option").remove();
            for(var i = 0; i < e.data.results.length; i++){
                var val = e.data.results[i];
                $("#loanApplicationUserId").append("<option value='"+val.userId+"'>"+val.userName+"</option>");
            }
