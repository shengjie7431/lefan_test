...
<body>
<div class="container">
    <form id="editForm" role="form" action="${ctx}/suning/withholdApply/okApply" method="post">
        <div class="form-group">
            <table class="table">
                <tbody>
                <input type="hidden" id="id" name="id" value="${id}">
                <input type="hidden" id="caseNo" name="id" value="${caseNo}">
                <input type="hidden" id="type" name="type" value="1">  <!-- 表示到账确认 -->
                <input type="hidden" id="applyType" name="applyType" value="${applyType}">
                <tr>
                    <th width="20%" class="active">到账金额</th>
                    <td width="80%">
                        <input type="number" step="0.01" required="required" min="1" class="form-control" name="commissionMoney"/>
                    </td>
                </tr>
                <tr>
                    <th width="20%" class="active">到账时间</th>
                    <td>
                        <input name="confirmAccountTime" type="text"  class="form-control" onclick="WdatePicker({dateFmt:'yyyy-MM-dd',minDate:'%y-%M-\#{%d-10}',maxDate:'%y-%M-%d'})" readonly>
                    </td>
                </tr>
                </tbody>
            </table>
