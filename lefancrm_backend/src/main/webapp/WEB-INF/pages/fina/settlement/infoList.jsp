<%--
  Created by IntelliJ IDEA.
  User: lixianfeng
  Date: 2020/12/7
  Time: 9:26
  To change this template use File | Settings | File Templates.
--%>
<%@ taglib prefix="form" uri="http://www.springframework.org/tags/form" %>
<%@ page language="java" pageEncoding="UTF-8" contentType="text/html;charset=UTF-8" %>
<%@ include file="/WEB-INF/pages/common/taglibs.jsp" %>
<html>
<head>
    <title>Title</title>
    <link rel="stylesheet" href="${ctx}/js/layui/css/layui.css">
    <link rel="stylesheet" href="${ctx}/css/viewer.min.css">
    <style>
        .records {
            width: 96%;
            margin: 20px auto;
        }

        .record-cell {
            width: 100%;
            display: flex;
            margin: 1px 0;
        }

        .cell-left {
            width: 30%;
            max-width: 200px;
            color: #666;
        }

        .cell-right {
            min-height: 80px;
            /*padding-bottom: 30px;*/
            width: 70%;
            border-left: 2px solid #3ba9fe;
        }

        /*.record-cell:last-of-type .cell-right{*/
            /*border-left: 2px solid #fff;*/
        /*}*/

        .icon-ricle {
            display: inline-block;
            width: 16px;
            height: 16px;
            border-radius: 16px;
            background: #3ba9fe;
            margin-left: -9px;
        }

        .userName {
            display: inline-block;
            width: 200px;
            padding: 0 20px;
            color: #3ba9fe;
        }

        .remark {
            width: 260px;
        }

        .wxui-ddr-btn {
            width: 100%;
            padding: 20px 0;
            background-color: #fff;
            position: fixed;
            bottom: 0;
            z-index: 1000;
        }

        .wxui-ddr-btn button {
            width: 92%;
            margin: 0 auto;
            height: 84px;
            line-height: 84px;
            background-color: #3BA9FF;
            color: #fff;
            font-size: 28px;
        }

        .cell-right-p {
            padding: 6px 0;
            padding-left: 46px;
        }

        .files {
            margin-top: 10px;
            width: 100%;
            display: flex;
            flex-wrap: wrap;
            /*max-height: 330px;*/
            overflow: auto;
        }

        .files .file {
            width: 60px;
            height: 60px;
            border: 1px solid #d0c9c9;
            margin-bottom: 20px;
            margin-right: 20px;
            position: relative;
        }

        .files .file .image {
            width: 100%;
            height: 100%;
            display: block;
            margin: 0 auto;

        }

        .files div.file-add {
            width: 60px;
            height: 60px;
            border: 1px dashed #d0c9c9;
            margin-bottom: 20px;
            margin-right: 20px;
        }

        .icon-add {
            margin-left: 30px;
            margin-top: 10px;
            display: inline-block;
            background: #aaa;
            height: 40px;
            position: relative;
            width: 2px;
        }

        .icon-add:after {
            background: #aaa;
            content: "";
            height: 40px;
            left: 0;
            position: absolute;
            top: 0;
            width: 2px;
            transform: rotateZ(90deg);
        }

        .file-title {
            position: absolute;
            bottom: 0;
            width: 100%;
            height: 20px;
            line-height: 20px;
            font-size: 10px;
            background-color: rgba(187, 187, 187, 0.7);
            white-space: nowrap;
            overflow: hidden;
            text-overflow: ellipsis;
        }

        .file-del {
            position: absolute;
            top: 2px;
            right: 2px;
            display: inline-block;
            width: 16px;
            height: 16px;
            background: url(data:image/png;base64,);
            background-position: center;
            background-repeat: no-repeat;
            background-size: cover;
        }

        .main {
            width: 100%;
            display: flex;
        }

        .main-left {
            width: 49%;
            height: 100%;
            overflow: auto;
            border-right: 1px solid #bbb
        }

        .main-title {
            margin: 10px auto;
            padding-left: 15px;
            width: 96%;
            overflow: auto;
            font-size: 18px;
            font-weight: bold;
        }

        .main-content {
            width: 96%;
            height: 100%;

            margin: 0 auto;
            overflow: auto;
        }

        .main-right {
            width: 49%;
            height: 100%;

        }
        .layui-form-label{
            text-align: left;
            width: 93px;
        }
        .layui-input, .layui-textarea, .files-content{
            width: 340px!important;
        }
        .poi-no{
            pointer-events: none;
        }
        .lf-none{
            width: 100%;
            color: #666;
            padding: 10px 0;
        }
        .lf-select-block {
            padding: 0 8px !important;
            white-space: nowrap;
            background-color: #3BA9FF;
        }
        @media screen and (min-width:1250px){
            .layui-form-label{
                text-align: left;
                width: 130px;
            }
            .layui-input, .layui-textarea, .files-content{
                width: 360px!important;
            }
        }
    </style>
</head>
<body>
<input type="hidden" value="${btnCode}" id="btnCode">
<input type="hidden" value="${dataUrge}" id="dataUrge">

<input type="hidden" value="${settlementId}" id="settlementId">

<div class="main">
    <div class="main-left">
        <div class="main-title"></div>
        <div class="main-content">
            <div class="records">

            </div>
        </div>
    </div>
    <div class="main-right">
        <div class="main-title"></div>
        <div class="main-content">
            <div class="layui-form">
                <c:if test="${btnCode == 'get-settlement-track'}">
                    <div class="layui-form-item">
                        <div class="layui-inline">
                            <label class="layui-form-label label-input" >跟踪内容</label>
                            <div class="layui-input-inline ">
                                <textarea class="layui-textarea trackDesc" placeholder="请输入跟踪内容"
                                          style="height: 150px;resize:none;"></textarea>
                            </div>
                        </div>
                    </div>
                    <div class="layui-form-item">
                        <div class="layui-inline">
                            <label class="layui-form-label label-input" >下一次跟踪时间</label>
                            <div class="layui-input-inline ">
                                <input type="text" class="layui-input nextTrackTime" readonly id="nextTrackTime"
                                       placeholder="请选择日期" >
                            </div>
                        </div>
                    </div>
                    <div class="layui-form-item">
                        <div class="layui-inline">
                            <label class="layui-form-label label-input" >上传附件</label>
                            <div class="layui-input-inline files-content" >
                                <div class='files' id="uploadFiles">
                                    <div class="file-add uploadFile">
                                        <div class="icon-add v-p-upload"></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </c:if>

                <c:if test="${btnCode == 'operate-bad' && dataUrge == '1'}">
                    <div class="layui-form-item">
                        <div class="layui-inline">
                            <label class="layui-form-label label-input" >联系人</label>
                            <div class="layui-input-inline ">
                                <input type="text" class="layui-input linkUser"
                                       placeholder="请输入" >
                            </div>
                        </div>
                    </div>
                    <div class="layui-form-item">
                        <div class="layui-inline">
                            <label class="layui-form-label label-input" >电话</label>
                            <div class="layui-input-inline ">
                                <input type="text" class="layui-input linkTel"
                                       placeholder="请输入电话号码" >
                            </div>
                        </div>
                    </div>
                    <div class="layui-form-item">
                        <div class="layui-inline">
                            <label class="layui-form-label label-input" >联系时间</label>
                            <div class="layui-input-inline ">
                                <input type="text" class="layui-input" readonly id="linkTime"
                                       placeholder="请选择日期" >
                            </div>
                        </div>
                    </div>
                    <div class="layui-form-item">
                        <div class="layui-inline">
                            <label class="layui-form-label label-input" >联系内容</label>
                            <div class="layui-input-inline ">
                                <textarea class="layui-textarea urgeDesc" placeholder="请输入联系内容"
                                          style="height: 150px;resize:none;"></textarea>
                            </div>
                        </div>
                    </div>
                    <div class="layui-form-item">
                        <div class="layui-inline">
                            <label class="layui-form-label label-input" >本次催收金额</label>
                            <div class="layui-input-inline ">
                                <input type="number" class="layui-input urgeMoney"
                                       placeholder="请输入金额" >
                            </div>
                        </div>
                    </div>
                    <div class="layui-form-item">
                        <div class="layui-inline">
                            <label class="layui-form-label label-input" >还款时间</label>
                            <div class="layui-input-inline ">
                                <input type="text" class="layui-input" readonly id="repaymentTime" placeholder="请选择日期" >
                            </div>
                        </div>
                    </div>
                    <div class="layui-form-item">
                        <div class="layui-inline">
                            <label class="layui-form-label label-input" >上传附件</label>
                            <div class="layui-input-inline files-content" >
                                <div class='files' id="uploadFiles">
                                    <div class="file-add uploadFile">
                                        <div class="icon-add v-p-upload"></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </c:if>

                <c:if test="${btnCode == 'operate-bad' && dataUrge == '2'}">
                    <div class="layui-form-item">
                        <div class="layui-inline">
                            <label class="layui-form-label label-input" >催收调查员</label>
                            <div class="layui-input-inline " >
                                <div id="surveyIds" class="selectMul" ></div>
                            </div>
                        </div>
                    </div>
                </c:if>

                <c:if test="${btnCode == 'operate-bad' && dataUrge == '3'}">
                    <div class="layui-form-item">
                        <div class="layui-inline">
                            <label class="layui-form-label label-input" >律师函</label>
                            <div class="layui-input-inline files-content" >
                                <div class='files' id="uploadFilesLSH">
                                    <div class="file-add uploadFile">
                                        <div class="icon-add v-p-upload"></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="layui-form-item">
                        <div class="layui-inline">
                            <label class="layui-form-label label-input" >本次催收金额</label>
                            <div class="layui-input-inline ">
                                <input type="number" class="layui-input urgeMoney"
                                       placeholder="请输入金额" >
                            </div>
                        </div>
                    </div>
                    <div class="layui-form-item">
                        <div class="layui-inline">
                            <label class="layui-form-label label-input" >还款时间</label>
                            <div class="layui-input-inline ">
                                <input type="text" class="layui-input" readonly id="repaymentTime"
                                       placeholder="请选择日期" >
                            </div>
                        </div>
                    </div>
                    <div class="layui-form-item">
                        <div class="layui-inline">
                            <label class="layui-form-label label-input" >上传附件</label>
                            <div class="layui-input-inline files-content" >
                                <div class='files' id="uploadFiles">
                                    <div class="file-add uploadFile">
                                        <div class="icon-add v-p-upload"></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </c:if>

                <c:if test="${btnCode == 'operate-bad' && dataUrge == '4'}">
                    <div class="layui-form-item">
                        <div class="layui-inline">
                            <label class="layui-form-label label-input" >诉讼材料</label>
                            <div class="layui-input-inline files-content" >
                                <div class='files' id="uploadFilesLSH">
                                    <div class="file-add uploadFile">
                                        <div class="icon-add v-p-upload"></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="layui-form-item">
                        <div class="layui-inline">
                            <label class="layui-form-label label-input" >本次催收金额</label>
                            <div class="layui-input-inline ">
                                <input type="number" class="layui-input urgeMoney"
                                       placeholder="请输入金额" >
                            </div>
                        </div>
                    </div>
                    <div class="layui-form-item">
                        <div class="layui-inline">
                            <label class="layui-form-label label-input" >还款时间</label>
                            <div class="layui-input-inline ">
                                <input type="text" class="layui-input" readonly id="repaymentTime"
                                       placeholder="请选择日期" >
                            </div>
                        </div>
                    </div>
                    <div class="layui-form-item">
                        <div class="layui-inline">
                            <label class="layui-form-label label-input" >上传附件</label>
                            <div class="layui-input-inline files-content" >
                                <div class='files' id="uploadFiles">
                                    <div class="file-add uploadFile">
                                        <div class="icon-add v-p-upload"></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </c:if>



                <div class="layui-form-item">
                    <div class="layui-input-block" style="margin-left: 140px;margin-top: 30px">
                        <button class="layui-btn layui-btn-primary close-btn" style="width: 100px;">取消</button>
                        <button class="layui-btn layui-btn-normal submit-btn" style="width: 100px;">确定</button>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>

<script src="${ctx}/js/layui/layui.js" charset="utf-8"></script>
<script src="${ctx}/js/jquery-3.4.1.js" charset="utf-8"></script>
<script src="${ctx}/js/viewer.min.js" charset="utf-8"></script>
<script>
    layui.config({
        base: '${ctx}/js/layui/layui_exts/'
    }).extend({
        xmSelect: 'xm-select'
    })

    layui.use(['upload', 'layer', 'laydate', 'util','xmSelect'], function () {
        var upload = layui.upload,
            laydate = layui.laydate,
            layer = layui.layer,
            xmSelect = layui.xmSelect,
            util = layui.util;

        var fullH = $(parent.document).height() * 0.9 - 70
        $('.main').height(fullH)
        $('.main-content').height(fullH - 46)


        var btnCode = $('#btnCode').val(),
            dataUrge = $('#dataUrge').val(),
            settlementId = $('#settlementId').val()

        var titleArray = [
            {
                id: 1,
                listTitle: '跟踪历史',
                addTitle:'添加跟踪'
            },
            {
                id: 2,
                listTitle: '电话催收历史',
                addTitle:'添加电话催收'
            },
            {
                id: 3,
                listTitle: '上门催收历史',
                addTitle:'添加上门催收'
            },
            {
                id: 4,
                listTitle: '律师函历史',
                addTitle:'上传律师函'
            },
            {
                id: 5,
                listTitle: '诉讼史',
                addTitle:'上传诉状等诉讼材料'
            }
        ]

        var initDate = getDate('today')

        var dataType = ''

        if (btnCode == 'get-settlement-track' ){
            dataType = 'get-settlement-track'
        }else  if (btnCode == 'operate-bad' ){
            dataType = 'get-urge-info-list-by-type'
        }

        $.ajax({
            url: '${ctx}/fina/settlement/ajaxData',
            data: {
                dataType: dataType,
                urgeType: dataUrge,
                settlementId: settlementId
            },
            success:function (res) {
                var res= JSON.parse(res)
                if (res.isSuccess){
                    setInfo(res)
                }else{

                }

            }
        })


        if (btnCode == 'get-settlement-track') {
            nextTrackTime = laydate.render({
                elem: '#nextTrackTime',
                value: initDate[0],
                min: initDate[0],
                type: 'datetime',
                format: 'yyyy-MM-dd HH:mm:ss',
                trigger: 'click'
            });
            $('.main-left .main-title').html(titleArray[0].listTitle)
            $('.main-right .main-title').html(titleArray[0].addTitle)
        }else if(btnCode == 'operate-bad' && dataUrge == '1'){
            linkTime = laydate.render({
                elem: '#linkTime',
                value:initDate[0],
                type: 'datetime',
                format: 'yyyy-MM-dd HH:mm:ss',
                trigger: 'click'
            });
            repaymentTime = laydate.render({
                elem: '#repaymentTime',
                value:initDate[0],
                min: initDate[0],
                type: 'datetime',
                format: 'yyyy-MM-dd HH:mm:ss',
                trigger: 'click'
            });
            $('.main-left .main-title').html(titleArray[1].listTitle)
            $('.main-right .main-title').html(titleArray[1].addTitle)
        }else if (btnCode == 'operate-bad' && dataUrge == '2'){
            demo1 = xmSelect.render({
                el: '#surveyIds',
                theme: {
                    color: '#3BA9FF',
                },
                size: 'small',
                radio: true,
                filterable: true,
                filterDone: function(val, list){
                    $('.xm-option-content').each(function () {
                        var _this = $(this)
                        _this.attr('title', _this.text())
                    })
                },
                toolbar: {
                    show: true,
                    list: ['CLEAR']
                },
                model: {
                    label: {
                        type: 'xxxx', //自定义与下面的对应
                        xxxx: {
                            template(data, sels) {
                                var _html = ''
                                sels.filter(function (cur) {
                                    _html +=
                                        '<div class="xm-label-block lf-select-block">' + cur
                                            .name + '</div>'
                                })
                                return _html
                            }
                        },
                    }
                },
                data: []
            })
            $.ajax({
                url: '${ctx}/fina/settlement/ajaxData',
                type: 'post',
                data: {
                    dataType: 'get-urge-users',
                    settlementId: $('#settlementId').val(),
                },
                success: function (res) {
                    var res = JSON.parse(res)
                    filterJson(demo1, res.results,'userId','userName',false, false)
                    // demo1.setValue([res.results[0].userId])
                }
            })
            $('.main-left .main-title').html(titleArray[2].listTitle)
            $('.main-right .main-title').html(titleArray[3].addTitle)
        }else if(btnCode == 'operate-bad' && dataUrge == '3'){
            repaymentTime = laydate.render({
                elem: '#repaymentTime',
                value:initDate[0],
                min: initDate[0],
                type: 'datetime',
                format: 'yyyy-MM-dd HH:mm:ss',
                trigger: 'click'
            });
            $('.main-left .main-title').html(titleArray[3].listTitle)
            $('.main-right .main-title').html(titleArray[3].addTitle)
        }else if(btnCode == 'operate-bad' && dataUrge == '4'){
            repaymentTime = laydate.render({
                elem: '#repaymentTime',
                value:initDate[0],
                min: initDate[0],
                type: 'datetime',
                format: 'yyyy-MM-dd HH:mm:ss',
                trigger: 'click'
            });
            $('.main-left .main-title').html(titleArray[4].listTitle)
            $('.main-right .main-title').html(titleArray[4].addTitle)
        }


        function setInfo(res) {
            var _html =''
            var list = []

            if (btnCode == 'get-settlement-track'){
                list = res.results.settlementTracks
                if (!list.length){
                    _html = '<div class="lf-none">暂无数据</div>'
                    $('.records').append(_html)
                    return;
                }
                list.map(function (cur,index) {
                    var _date = cur.trackFtime ? util.toDateString(cur.createTime,'yyyy-MM-dd HH:mm') : ''
                    var _htmlFile = ''
                    cur.finaFiles.map(function (item) {
                       _htmlFile += ' <div class="file">\n' +
                           '                        <img src="' + item.filePath + '" class="image"></img>\n' +
                        '                    </div>'
                    })
                    _html += '<div class="record-cell">\n' +
                        '                    <div class="cell-left">'+_date+'</div>\n' +
                        '                    <div class="cell-right">\n' +
                        '                        <div>\n' +
                        '                            <div class="icon-ricle"></div>\n' +
                        '                            <div class="userName">（'+cur.trackUserName+'）</div>\n' +
                        '                        </div>\n' +
                        '                        <div class="cell-right-p">\n' +cur.trackDesc+
                        '                        </div>\n' +
                        '                        <div class="cell-right-p">\n' +
                        '                            <div>凭证：</div>\n' +
                        '                            <div class="files" id="uploadFiles'+index+'" >\n' +
                                                        _htmlFile+
                        '                            </div>\n' +
                        '                        </div>\n' +
                        '                    </div>\n' +
                        '                </div>'

                })
            }else if(btnCode == 'operate-bad' && dataUrge == '1'){
                list = res.results.urgeInfos
                if (!list.length){
                    _html = '<div class="lf-none">暂无数据</div>'
                    $('.records').append(_html)
                    return;
                }
                list.map(function (cur,index) {
                    var _date = cur.createTime ? util.toDateString(cur.createTime, 'yyyy-MM-dd HH:mm') : ''

                    var repaymentTime = cur.repaymentTime ? util.toDateString(cur.repaymentTime, 'yyyy-MM-dd HH:mm') : ''
                    var _htmlFile = ''
                    cur.finaFiles.map(function (item) {
                        _htmlFile += ' <div class="file">\n' +
                            '                        <img src="' + item.filePath + '" class="image"></img>\n' +
                            '                    </div>'
                    })
                    _html +='<div class="record-cell">\n' +
                    '                    <div class="cell-left">'+_date+'</div>\n' +
                    '                    <div class="cell-right">\n' +
                    '                        <div>\n' +
                    '                            <div class="icon-ricle"></div>\n' +
                    '                            <div class="userName">（'+cur.createBy+'）</div>\n' +
                    '                        </div>\n' +
                    '                        <div class="cell-right-p">\n' +
                    '                            联系人：' + cur.linkUser+
                    '                        </div>\n' +
                    '                        <div class="cell-right-p">\n' +
                    '                            电话：' + cur.linkTel+
                    '                        </div>\n' +
                    '                        <div class="cell-right-p">\n' +
                    '                            联系内容：' + cur.urgeDesc+
                    '                        </div>\n' +
                    '                        <div class="cell-right-p">\n' +
                    '                            本次催收金额：' + cur.urgeMoney+
                    '                        </div>\n' +
                    '                        <div class="cell-right-p">\n' +
                    '                            还款时间：' + repaymentTime+
                    '                        </div>\n' +
                    '                        <div class="cell-right-p">\n' +
                    '                            <div>凭证：</div>\n' +
                    '                            <div class="files" id="uploadFiles'+index+'" >\n' +
                    _htmlFile+
                    '                            </div>\n' +
                    '                        </div>\n' +
                    '                    </div>\n' +
                    '                </div>'

                })
            }else if(btnCode == 'operate-bad' && dataUrge == '2'){
                list = res.results.urgeInfos
                if (!list.length){
                    _html = '<div class="lf-none">暂无数据</div>'
                    $('.records').append(_html)
                    return;
                }
                list.map(function (cur,index) {
                    var _date = cur.createTime ? util.toDateString(cur.createTime, 'yyyy-MM-dd HH:mm') : ''

                    var repaymentTime = cur.repaymentTime ? util.toDateString(cur.repaymentTime, 'yyyy-MM-dd HH:mm') : ''
                    var _htmlFile = ''
                    cur.finaFiles.map(function (item) {
                        _htmlFile += ' <div class="file">\n' +
                            '                        <img src="' + item.filePath + '" class="image"></img>\n' +
                            '                    </div>'
                    })
                    _html +='<div class="record-cell">\n' +
                        '                    <div class="cell-left">'+_date+'</div>\n' +
                        '                    <div class="cell-right">\n' +
                        '                        <div>\n' +
                        '                            <div class="icon-ricle"></div>\n' +
                        '                            <div class="userName">（'+cur.createBy+'）</div>\n' +
                        '                        </div>\n' +
                        '                        <div class="cell-right-p">\n' +
                        '                            催收调查员：' + cur.urgeUserName+
                        '                        </div>\n' +
                        '                        <div class="cell-right-p">\n' +
                        '                            催收详情：' + (cur.urgeDesc == null ? '' : cur.urgeDesc) +
                        '                        </div>\n' +
                        '                        <div class="cell-right-p">\n' +
                        '                            本次催收金额：' + (cur.urgeMoney == null ? '' : cur.urgeMoney) +
                        '                        </div>\n' +
                        '                        <div class="cell-right-p">\n' +
                        '                            还款时间：' + repaymentTime+
                        '                        </div>\n' +
                        '                        <div class="cell-right-p">\n' +
                        '                            <div>凭证：</div>\n' +
                        '                            <div class="files" id="uploadFiles'+index+'" >\n' +
                        _htmlFile+
                        '                            </div>\n' +
                        '                        </div>\n' +
                        '                    </div>\n' +
                        '                </div>'

                })
            }else if(btnCode == 'operate-bad' && dataUrge == '3' || btnCode == 'operate-bad' && dataUrge == '4'){
                list = res.results.urgeInfos
                console.log(list,list.length)
                if (!list.length){
                    _html = '<div class="lf-none">暂无数据</div>'
                    $('.records').append(_html)
                    return;
                }
                var lsName = ''
                if (dataUrge == '3'){
                    lsName = '律师函'
                }else if (dataUrge == '4'){
                    lsName = '诉讼材料'
                }

                list.map(function (cur,index) {
                    var _date = cur.createTime ? util.toDateString(cur.createTime, 'yyyy-MM-dd HH:mm') : ''

                    var repaymentTime = cur.repaymentTime ? util.toDateString(cur.repaymentTime, 'yyyy-MM-dd HH:mm') : ''
                    var _htmlFile = ''
                    cur.finaFiles.map(function (item) {
                        _htmlFile += ' <div class="file">\n' +
                            '                        <img src="' + item.filePath + '" class="image"></img>\n' +
                            '                    </div>'
                    })

                    var _htmlOneFile = ''
                    cur.oneFiles.map(function (item) {
                        _htmlOneFile += ' <div class="file">\n' +
                            '                        <img src="' + item.filePath + '" class="image"></img>\n' +
                            '                    </div>'
                    })
                    _html +='<div class="record-cell">\n' +
                        '                    <div class="cell-left">'+_date+'</div>\n' +
                        '                    <div class="cell-right">\n' +
                        '                        <div>\n' +
                        '                            <div class="icon-ricle"></div>\n' +
                        '                            <div class="userName">（'+cur.createBy+'）</div>\n' +
                        '                        </div>\n' +
                        '                        <div class="cell-right-p">\n' +
                        '                            <div>'+lsName+'：</div>\n' +
                        '                            <div class="files" id="uploadFilesOne'+index+'" >\n' +
                        _htmlOneFile+
                        '                            </div>\n' +
                        '                        </div>\n' +
                        '                        <div class="cell-right-p">\n' +
                        '                            本次催收金额：' + cur.urgeMoney+
                        '                        </div>\n' +
                        '                        <div class="cell-right-p">\n' +
                        '                            还款时间：' + repaymentTime+
                        '                        </div>\n' +
                        '                        <div class="cell-right-p">\n' +
                        '                            <div>凭证：</div>\n' +
                        '                            <div class="files" id="uploadFiles'+index+'" >\n' +
                        _htmlFile+
                        '                            </div>\n' +
                        '                        </div>\n' +
                        '                    </div>\n' +
                        '                </div>'

                })
            }


            $('.records').append(_html)
            for(var i =0;i<list.length;i++){
                if (list[i].finaFiles.length){
                    viewer = new Viewer(document.getElementById('uploadFiles'+i));
                }
                if (list[i].oneFiles && list[i].oneFiles.length){
                    viewer = new Viewer(document.getElementById('uploadFilesOne'+i));
                }
            }

        }


        $('.submit-btn').click(function () {
            var _this = $(this)


            var params = {}
            var dataUrgeCode = ''

            if (btnCode == 'operate-bad'){
                if (dataUrge == 1 ){
                    dataUrgeCode = 'tel-urge'
                }else  if (dataUrge == 2 ){
                    dataUrgeCode = 'address-urge'
                }else if (dataUrge == 3 ){
                    dataUrgeCode = 'ls-urge'
                }else if (dataUrge == 4 ){
                    dataUrgeCode = 'ss-urge'
                }else if (dataUrge == 5 ){
                    dataUrgeCode = 'stop-urge'
                }
            }

            if (btnCode == 'get-settlement-track') {
                var files= []
                $('#uploadFiles .file').map(function (i,cur) {
                    files.push({
                        filePath: $(cur).find('img').attr('src')
                    })
                })
                params = {
                    btnCode: 'settlement-track-save',
                    settlementId: $('#settlementId').val(),
                    trackDesc: $('.trackDesc').val(),
                    files: JSON.stringify(files),
                    nextTrackTime: $('.nextTrackTime').val()
                }
                if (!params.trackDesc){
                    layer.alert('请输入跟踪内容！', {
                        icon: 5
                    })
                    return;
                }


                if (!params.nextTrackTime){
                    layer.alert('请选择下一次跟踪时间！', {
                        icon: 5
                    })
                    return;
                }

                if (!files.length){
                    layer.alert('请输入上传附件！', {
                        icon: 5
                    })
                    return;
                }
            }else if (btnCode == 'operate-bad' && dataUrge == '1') {
                var files= []
                $('#uploadFiles .file').map(function (i,cur) {
                    files.push({
                        filePath: $(cur).find('img').attr('src')
                    })
                })
                params = {
                    btnCode: 'operate-bad',
                    settlementId: $('#settlementId').val(),
                    urgeType: dataUrgeCode,
                    linkUser: $('.linkUser').val(),
                    linkTel: $('.linkTel').val(),
                    linkTime: $('#linkTime').val(),
                    urgeDesc: $('.urgeDesc').val(),
                    urgeMoney: $('.urgeMoney').val(),
                    repaymentTime: $('#repaymentTime').val(),
                    files:JSON.stringify(files)
                }
                if (!params.linkUser){
                    layer.alert('请输入联系人！', {
                        icon: 5
                    })
                    return;
                }
                if (!params.linkTel){
                    layer.alert('请输入电话号码！', {
                        icon: 5
                    })
                    return;
                }
                if (!params.linkTime){
                    layer.alert('请选择联系时间！', {
                        icon: 5
                    })
                    return;
                }
                if (!params.urgeDesc){
                    layer.alert('请输入联系内容！', {
                        icon: 5
                    })
                    return;
                }
                if (!params.urgeMoney){
                    layer.alert('请输入本次催收金额！', {
                        icon: 5
                    })
                    return;
                }
                if (params.urgeMoney && !checkPapers('money',params.urgeMoney)){
                    layer.alert('请输入正确格式的还款金额,已修改', {
                        icon: 5
                    })
                    return;
                }

                if (!params.repaymentTime){
                    layer.alert('请选择还款时间！', {
                        icon: 5
                    })
                    return;
                }

                if (!files.length){
                    layer.alert('请输入上传附件！', {
                        icon: 5
                    })
                    return;
                }
            }else if (btnCode == 'operate-bad' && dataUrge == '2') {
                params = {
                    btnCode: 'operate-bad',
                    settlementId: $('#settlementId').val(),
                    urgeType: dataUrgeCode,
                    urgeUserId: demo1.getValue('valueStr'),
                }
                if (!params.urgeUserId){
                    layer.alert('请选择催收调查员！', {
                        icon: 5
                    })
                    return;
                }

            }else if (btnCode == 'operate-bad' && dataUrge == '3') {
                var lsFilesOne= [],lsFilesTwo = []

                $('#uploadFilesLSH .file').map(function (i,cur) {
                    lsFilesOne.push({
                        filePath: $(cur).find('img').attr('src')
                    })
                })
                $('#uploadFiles .file').map(function (i,cur) {
                    lsFilesTwo.push({
                        filePath: $(cur).find('img').attr('src')
                    })
                })
                params = {
                    btnCode: 'operate-bad',
                    settlementId: $('#settlementId').val(),
                    urgeType: dataUrgeCode,
                    urgeMoney: $('.urgeMoney').val(),
                    repaymentTime: $('#repaymentTime').val(),
                    lsFilesOne: JSON.stringify(lsFilesOne),
                    lsFilesTwo:JSON.stringify(lsFilesTwo)
                }

                if (!params.urgeMoney){
                    layer.alert('请输入本次催收金额！', {
                        icon: 5
                    })
                    return;
                }
                if (params.urgeMoney && !checkPapers('money',params.urgeMoney)){
                    layer.alert('请输入正确格式的还款金额!', {
                        icon: 5
                    })
                    return;
                }

                if (!params.repaymentTime){
                    layer.alert('请选择还款时间！', {
                        icon: 5
                    })
                    return;
                }

                if (!lsFilesOne.length){
                    layer.alert('请上传律师函！', {
                        icon: 5
                    })
                    return;
                }
                if (!lsFilesTwo.length){
                    layer.alert('请上传凭证附件！', {
                        icon: 5
                    })
                    return;
                }

            }else if (btnCode == 'operate-bad' && dataUrge == '4') {
                var lsFilesOne= [],lsFilesTwo = []

                $('#uploadFilesLSH .file').map(function (i,cur) {
                    lsFilesOne.push({
                        filePath: $(cur).find('img').attr('src')
                    })
                })
                $('#uploadFiles .file').map(function (i,cur) {
                    lsFilesTwo.push({
                        filePath: $(cur).find('img').attr('src')
                    })
                })
                params = {
                    btnCode: 'operate-bad',
                    settlementId: $('#settlementId').val(),
                    urgeType: dataUrgeCode,
                    urgeMoney: $('.urgeMoney').val(),
                    repaymentTime: $('#repaymentTime').val(),
                    ssFilesOne: JSON.stringify(lsFilesOne),
                    ssFilesTwo:JSON.stringify(lsFilesTwo)
                }

                if (!params.urgeMoney){
                    layer.alert('请输入本次催收金额！', {
                        icon: 5
                    })
                    return;
                }
                if (params.urgeMoney && !checkPapers('money',params.urgeMoney)){
                    layer.alert('请输入正确格式的还款金额!', {
                        icon: 5
                    })
                    return;
                }

                if (!params.repaymentTime){
                    layer.alert('请选择还款时间！', {
                        icon: 5
                    })
                    return;
                }

                if (!lsFilesOne.length){
                    layer.alert('请上传诉讼材料！', {
                        icon: 5
                    })
                    return;
                }
                if (!lsFilesTwo.length){
                    layer.alert('请上传凭证附件！', {
                        icon: 5
                    })
                    return;
                }

            }

            _this.addClass('poi-no')
            $.ajax({
                url: '${ctx}/fina/settlement/operate',
                type: 'post',
                data: params,
                success: function (res) {
                    var res = JSON.parse(res)
                    if (res.isSuccess){
                        layer.msg('成功', {
                            time: 1000,
                            icon: 1
                        },function(){
                            parent.location.reload()
                        })
                    }else{
                        _this.removeClass('poi-no')
                        layer.msg(res.msg, {
                            time: 1000,
                            icon: 2
                        })
                    }
                }
            })
        })
        $('.close-btn').click(function () {
            closeDialog()
        })

        var uploadFile = upload.render({
            elem: '.uploadFile',
            url: '${ctx}/fina/files/uploadSftp', //改成您自己的上传接口
            data: {
                modelType: 'settlementUrge',
                pathId: $('#settlementId').val(),
            },
            multiple: true,
            accept: 'images', //只能上传图片
            acceptMime: 'image/*',
            before: function (obj) {
                layer.load();
                nowId = $(this.item).parent().attr('id')
                fileLen = $('#'+nowId).find('.file').length
            },
            done: function (res) {
                layer.closeAll('loading')
                if (res.success == 'true') {
                    putSessionFiles(res.surveyFile,$(this.item).parent().attr('id'));
                } else {
                    layer.alert('导入出错', {
                        icon: 2
                    })
                }
            },
            allDone: function (obj) {
                if (fileLen && nowId == 'uploadFiles') {
                    viewer1.destroy()
                }
                if (fileLen && nowId == 'uploadFilesLSH') {
                    viewer2.destroy()
                }

                if ($('#uploadFiles .file').length && nowId == 'uploadFiles'){
                    viewer1 = new Viewer(document.getElementById('uploadFiles'));
                }
                if ($('#uploadFilesLSH .file').length && nowId == 'uploadFilesLSH'){
                    viewer2 = new Viewer(document.getElementById('uploadFilesLSH'));
                }
            },
            error: function (index, upload) {
                layer.closeAll('loading')
            }
        });


        $('#uploadFiles').on('click','.file-del',function () {
            $(this).parent().detach()

            viewer1.destroy()
            if ($('#uploadFiles .file').length){
                viewer1 = new Viewer(document.getElementById('uploadFiles'));
            }
        })

        $('#uploadFilesLSH').on('click','.file-del',function () {
            $(this).parent().detach()

            viewer2.destroy()
            if ($('#uploadFilesLSH .file').length){
                viewer2 = new Viewer(document.getElementById('uploadFilesLSH'));
            }
        })


        function putSessionFiles(surveyFile,id) {
            var _html = ' <div class="file">\n' +
                '                        <img src="' + surveyFile.filePath + '" class="image"></img>\n' +
                // '<div class="file-title">' + surveyFile.fileName + '</div>\n' +
                '                        <div class="file-del"></div>'
            '                    </div>'
            $('#'+id).append(_html)
        }

        function getDate(id) {
            var today = new Date(),
                y = today.getFullYear(),
                m = today.getMonth() + 1,
                d = today.getDate(),
                w = today.getDay(),
                millisecond = 1000 * 60 * 60 * 24;
            if (id == 'upMonth') {
                var y1 = y,
                    m1 = m
                if (m == 1) {
                    y1 = y - 1
                    m1 = 12
                } else {
                    m1 = m - 1
                }
                m1 = PrefixInteger(m1, 2)
                var days1 = new Date(y1, m1, 0).getDate()
                vals = [y1 + '-' + m1 + '-01', y1 + '-' + m1 + '-' + days1]
            } else if (id == 'yesterday') {
                var yesterDay = new Date(today.getTime() - millisecond);
                vals = [dateFormat(yesterDay), dateFormat(yesterDay)]
            } else if (id == 'today') {
                var m1 = PrefixInteger(m, 2)
                var d1 = PrefixInteger(d, 2)
                vals = [y + '-' + m1 + '-' + d1 +' 00:00:00', y + '-' + m1 + '-' + d1]
            } else if (id == 'curWeek') {
                var minusDay = w != 0 ? w - 1 : 6;
                var monday = new Date(today.getTime() - (minusDay * millisecond));
                var sunday = new Date(monday.getTime() + (6 * millisecond));
                vals = [dateFormat(monday), y + '-' + m + '-' + d]
            } else if (id == 'curMonth') {
                vals = [y + '-' + m + '-01', y + '-' + m + '-' + d]
            } else if (id == 'all') {
                vals = ['2019-02-27', y + '-' + m + '-' + d]
            }
            return vals
        }

        function dateFormat(time, format) {
            var t = new Date(time);
            var format = format || 'yyyy-MM-dd'
            var tf = function (i) {
                return (i < 10 ? '0' : '') + i
            };
            return format.replace(/yyyy|MM|dd|HH|mm|ss/g, function (a) {
                switch (a) {
                    case 'yyyy':
                        return tf(t.getFullYear());
                        break;
                    case 'MM':
                        return tf(t.getMonth() + 1);
                        break;
                    case 'mm':
                        return tf(t.getMinutes());
                        break;
                    case 'dd':
                        return tf(t.getDate());
                        break;
                    case 'HH':
                        return tf(t.getHours());
                        break;
                    case 'ss':
                        return tf(t.getSeconds());
                        break;
                }
            })
        };

        function PrefixInteger(num, m) {
            return (Array(m).join(0) + num).slice(-m);
        }
        function closeDialog(){
            var closeBtn = $("#diglog_close_btn");
            if(closeBtn.length == 0){
                closeBtn = $("#diglog_close_btn",window.parent.document);
            }
            closeBtn.click();
        }

        function filterJson(demo, newJson, id, name, flag, selected) {
            var demoList = [],
                demoValues = []
            newJson.map(function (cur) {
                var _name = name ? cur[name] : cur.name
                var _id = id ? cur[id] : cur.id
                var param = {
                    name: _name,
                    value: _id,
                }
                if (selected) {
                    Object.assign(param, {
                        selected: true
                    })
                }
                if (cur.checked){
                    Object.assign(param, {
                        selected: true
                    })
                }
                demoList.push(param)
                demoValues.push(_id)
            })
            if (!flag) {
                demo.update({
                    data: demoList
                })
            } else {
                return demoList
            }
            setTimeout(function () {
                $('.xm-option-content').each(function () {
                    var _this = $(this)
                    _this.attr('title', _this.text())
                })
            }, 200)
        }
        //正则匹配
        var checkPapers = function (parama, paramb) {
            var map = new Map([
                ['phone', /^[1][3,4,5,7,8,9][0-9]{9}$/],
                ['passport', !/^((1[45]\d{7})|(G\d{8})|(P\d{7})|(S\d{7,8}))?$/],
                ['identity', /(^\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/],
                ['money', /^[0-9]+(\.[0-9]{1,2})?$/],
                ['moneyorMinus', /^(\-|\+)?\d+(\.\d{1,2})?$/],
                ['integer', /^[0-9]\d*$/],
                ['email',/^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/],
                ['num_0_1',/^(0+(\.[0-9]{1,2})?|1|1.0|1.00?)$/]
            ])

            if (map.get(parama).test(paramb)) {
                return true
            } else {
                return false
            }
        }


    })
</script>
</body>
</html>
