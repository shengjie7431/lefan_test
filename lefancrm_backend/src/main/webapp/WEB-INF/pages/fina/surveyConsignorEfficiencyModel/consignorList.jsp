...
<div class="main administrator">
    ...
    <div class="panel-heading">
        <div class="pin">
            <form class="form-inline" role="form" action="${ctx}/finaManager/popup?surveyCode=${surveyCode}&id=${id}&btnCode=${btnCode}" method="post"  onsubmit="return false;">
    ...
    <div class="panel panel-info">
        <form id="editForm" role="form" action="${ctx}/finaManager/operate" method="post">
            <input type="hidden" name="efficiencyModelId" value="${efficiencyModelId}">
                ...
                <tr>
                    <td>
                        <input type="checkbox" name="buss" value="${item.id}"
                            <c:forEach items="${modelOrgs}" var="my">
                                <c:if test="${my.orgId == item.id }">
                                    checked="checked"
                                </c:if>
                            </c:forEach>
                        />
                    ...
                    <td class="companyName">
                        ${item.company}
                            <c:forEach items="${modelOrgAll}" var="all">
                                <c:if test="${all.orgId == item.id }">
                                    <span style="color: #b3aaaa">(${all.efficiencyModelName})</span>
                                </c:if>
                            </c:forEach>
                    </td>
                </tr>
