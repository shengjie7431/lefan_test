    ...
    <div class="row">
        ...
        <div class="col-sm-10 " id="result">
            <div class="row panel panel-info">
                ...
                <div class="panel-heading">
                    <div class="pin">
                        <form class="form-inline" role="form"  method="post" action="${ctx}/org/moneyOut"><%--action="${ctx}/case/caseCenterInfoList"--%>
                            <div class="form-group">
                            ...
                            月份晒选： <input id="startMonth" name="startMonth" onchange="clearMouth(2)" type="text" value="${startMonth}" class="form-control" onclick="WdatePicker({dateFmt:'yyyy-MM'})" readonly>
                            <input id="endMonth" name="endMonth" onchange="clearMouth(2)" type="text" value="${endMonth}" class="form-control" onclick="WdatePicker({dateFmt:'yyyy-MM'})" readonly>
                            ...
                            <%--<div class="btn-group">
                                &nbsp; &nbsp;<button id="queryBtn" class="btn btn-default" onclick="query('')">查询</button>&nbsp; &nbsp;
                            </div>--%>
...
//		},
    ...
    function zTreeOnClick(e, treeId, treeNode){
        ...
        $.ajax({
            url: '${ctx}/org/queryOrgMoneyListById?orgId='+treeNode.id+'&startMonth='+$("#startMonth").val()+'&endMonth='+$("#endMonth").val()+'&naturalMonth='+$("#naturalMonth").val(),
            method:'POST',
            success: function(res){
                var json = JSON.parse(res);
                var orgNames=[];    //类别数组（实际用来盛放X轴坐标值）
...
//                $("#naturalMonth").val(json.naturalMonth);
                ...
                // echart
                // echart显示
                setEchart(orgNames,saleAmounts,saleGoals,completionRates);
            }
        })
    }
...
//        console.info($("#startDate").val());
        $("table tbody tr").remove();
        $.ajax({
            url: '${ctx}/org/queryOrgMoneyListById',
            method:'POST',
            data:'orgId='+ $("#orgId").val()+'&startMonth='+$("#startMonth").val()+'&endMonth='+$("#endMonth").val()+'&naturalMonth='+$("#naturalMonth").val()+'&orderByCompletionRate='+orderByCompletionRate,//+'&orderByCompletionRate='+$("#orderByVisitNum").val()+'&orderBySignNum='+$("#orderBySignNum").val(),
            success: function(res){
...
function setEchart(orgNames, saleAmounts,saleGoals,completionRates){
    var myChart = echarts.init(document.getElementById('myCharts'));
    //console.log(myChart)
