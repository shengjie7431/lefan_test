...
<div class="container">
    <form id="editForm" role="form" action="${ctx}/billingApply/billingApplySave" method="post">
        ...
        <c:if test="${copy == '4'}">
            <input type="hidden" name="billSource" value="df" /> <%-- 开票来源为 调查 --%>
            <input type="hidden" name="billingSource" value="6" /> <%--默认未狄大人开票--%>
        </c:if>
        <c:if test="${copy != '4'}">
            <input type="hidden" name="billSource" value="survey" /> <%-- 开票来源为 调查 --%>
            <input type="hidden" id="copy" name="copy" value="${copy}" /> <%-- 开票来源为 调查 --%>
            <input type="hidden" name="billingSource" value="3" /> <%--默认未狄大人开票--%>
        </c:if>
        <input type="hidden" id="idList" name="idList" value="${idList}" /><%-- 批量开票的id集合 --%>
        ...
        <input type="hidden" id="entrustOrgId" name="entrustOrgId" value="${entrustOrgId}">
        ...
        <div class="form-group">
            <table class="table">
                ...
                <tr>
                    <th width="20%" class="active">委托方机构</th>
                    <td width="80%">
                        <input type="text" id="surveyConsignorName" name="surveyConsignorName" value="${companyName}" style="width: 400px; display: inline-block;" class="form-control" readonly="true">
                        <input type="hidden" id="surveyConsignorId" name="surveyConsignorId" value="${companyId}" >
                ...
                <tr>
                    ...
                    <td width="80%">
                        <input type="text" id="companyName" name="companyName" value="${billingApplyInfo.companyName}" style="width: 322px; display: inline-block;" class="form-control" placeholder="可输入">
                        <input type="hidden" id="companyId" name="companyId" value="${billingApplyInfo.companyId}">
                        <input type="button" value="选  择" onclick="selectCompanyName()"  style="width: 78px;  height: 32px;"/>
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td width="80%">
                        <select name="staffOrgId" id="staffOrgId" class="form-control" style="width: 400px;" required="required" onchange="selectBillingEnum()">
                            ...
                            <option value="${item.id}" <c:if test="${item.id == billingApplyInfo.staffOrgId}">selected = "selected"</c:if> >${item.name}</option>
                            </c:forEach>
                        </select>
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <input type="hidden" id="province" name="province" value="${billingApplyInfo.province}">
                    <input type="hidden" id="city" name="city" value="${billingApplyInfo.city}">
                    <input type="hidden" id="district" name="district"  value="${billingApplyInfo.district}">
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td  width="80%">
                        <input type="text" id="insuredName" name="insuredName" value="" style="width: 400px;" class="form-control">
                    </td>
                </tr>
                <tr>
                    <th width="20%" class="active">车牌号</th>
                    <td  width="80%">
                        <input type="text" id="carNo" name="carNo" value="" style="width: 400px;" class="form-control">
                    </td>
                </tr>
                <tr>
                    <th width="20%" class="active">伤者姓名</th>
                    <td  width="80%">
                        <input type="text" id="woundedName" name="woundedName" value="" style="width: 400px;" class="form-control">
                    </td>
                </tr>
<%--                <tr>--%>
<%--                    <th width="20%" class="active">开票主体</th>--%>
<%--                    <td width="80%">--%>
<%--                        <select name="surveyBillSubjectId" id="surveyBillSubjectId" class="form-control" style="width: 400px;">--%>
<%--                            <option value="">请选择</option>--%>
<%--                            <c:forEach items="${billSubjectDtoList}" var="item">--%>
<%--                                <option value="${item.id}" <c:if test="${billingApplyInfo.surveyBillSubjectId == item.id}"> selected="selected" </c:if>>${item.name}</option>--%>
<%--                            </c:forEach>--%>
<%--                        </select>--%>
<%--                    </td>--%>
<%--                </tr>--%>
                ...
                <tr style="display: none;">
                    ...
                    <td width="80%">
                        <select name="surveyDepartmentId" class="form-control" style="width: 400px;">
                            <option value="">全部</option>
                            <c:forEach items="${departmentDto}" var="item">
                ...
                <tr>
                    ...
                    <td  width="80%">
                        <input type="text" id="policyNo" name="policyNo" value="" style="width: 400px;" class="form-control">
                    </td>
                </tr>
                <tr>
                    <th width="20%" class="active">报案号</th>
                    <td  width="80%">
                        <input type="text" id="reportNo" name="reportNo" value="" style="width: 400px;" class="form-control">
                    </td>
                </tr>
                <tr>
                    <th width="20%" class="active">备注</th>
                    <td  width="80%">
                        <input type="textarea" id="remark" name="remark" value="" style="width: 400px;" class="form-control">
                    </td>
                </tr>
...
<script type="text/javascript">
    ...
    function init(){
        if ($("#copy").val() == 3){
            $("#idList").val(window.parent.$("#surveyInfoIds").val());
        }
    }
...
//                $("#materialImgsDiv").append("<img width='80' height='80' src='http://ddrapi.shlefan.com/sftp/files/"+pathImg+"' />");
    ...
    function toBuildCaseNo(){
        ...
        <%--$.ajax({--%>
            ...
            <%--success:function(res,param){--%>
                ...
                <%--<%--$("#billingFileupload").attr("data-url","${ctx}/uploadImage?moduleName=ticket/"+param.data.results);--%>
                <%--<%--$("#materialFileupload").attr("data-url","${ctx}/uploadImage?moduleName=ticket/"+param.data.results);--%>
            <%--}--%>
        <%--});--%>
    }
    ...
    function selectBillingEnum(){
        ...
        ajaxSubmit("${ctx}/billingApply/selectInfoByRelationId",{"organId":organId,btnCode:1100},function(v,e,p){
            ...
            var len = e.data.results.length;
            for(var i = 0; i < len; i++){
                var val = e.data.results[i];
                if(len == 1){
                    $("#billingEnum").append("<option value='" + val.productEnumId + "' selected='selected'>" + val.productEnumName + "</option>");
                }else{
    ...
    function selectBillingItem(){
        ...
        ajaxSubmit("${ctx}/billingApply/selectInfoByRelationId",{"billingEnum":billingEnum,btnCode:1200},function(v,e,p){
            ...
            for(var i = 0; i < e.data.results.length; i++){
                var val = e.data.results[i];
                if(len == 1){
                    $("#billingItem").append("<option value='" + val.billingItemId + "' selected='selected'>" + val.billingItemName + "</option>");
                }else{
