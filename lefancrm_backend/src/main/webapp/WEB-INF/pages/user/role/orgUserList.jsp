...
<div class="main administrator">
    ...
    <div class="panel panel-info">
        ...
        <div class="panel-heading">
            <div class="pin">
                <form class="form-inline" role="form" action="${ctx}/user/role/queryUser" method="post">
   ...
   <%-- <div class="panel-heading">
                ...
                <tr>
                    ...
                    <td>
                        <%--<c:if test="${isRole == 1}">--%>
                            ...
                            <%--<a  href="javascript:void(0)" onclick="delDsRole(${item.userId})">移除用户</a>--%>
                            <%--<c:if test="${orgType == 2}">--%>
                                <%--<a  href="javascript:void(0)" onclick="insUserAddressList(${item.userId})">查看辖区</a>--%>
                            <%--</c:if>--%>
                                <%--<c:forEach items="${item.busUserRoles}" var="roles">--%>
                                    <%--<c:if test="${roles.roleId ==9}">--%>
                                        <%--<a  href="javascript:void(0)" onclick="insuranceList(${item.userId},${item.orgId})">添加保险员</a>--%>
                                    <%--</c:if>--%>
                                <%--</c:forEach>--%>
                        <%--</c:if>--%>
                    </td>
    ...
    <div class="main-bottom">
        <jsp:include page="/WEB-INF/pages/common/pagination.jsp" flush="true">
            ...
            <jsp:param name="requestUrl" value="${ctx}/user/role/queryUser?id=${id}&userName=${userName}&nickName=${nickName}&userTel=${userTel}" />
...
<script>
    ...
    var disDsRole = function(userId){
        openDialog({
            ...
            height:500,
            width:1000,
            url:"${ctx}/user/role/disDsRole?userId="+userId
        });
    }

    function delDsRole(userId){
        ajaxSubmit("${ctx}/org/delOrgUser",{"userId":userId},reload,"移除成功！","确定将该用户移除机构？",null);
    }
