            ...
            $main.each(function (i) {
                for (var item in mergeRecord) {
                    if (i===$main.length-1 || isMaster(i, item)) {
                        ...
                        tdHeight = 39
                        $(this).children('[data-key="'+item+'"]').attr('rowspan', mergeRecord[item].rowspan).css({'position': 'static', 'height': tdHeight*mergeRecord[item].rowspan + patchHeight}).children().css({height: '-webkit-fill-available', 'white-space': 'normal', 'max-height': tdHeight*mergeRecord[item].rowspan + patchHeight - 10, 'line-height': tdHeight*mergeRecord[item].rowspan + patchHeight - 10 + 'px'});
                        $fixLeft.eq(i).children('[data-key="'+item+'"]').attr('rowspan', mergeRecord[item].rowspan).css({'position': 'static', 'height': tdHeight*mergeRecord[item].rowspan + patchHeight}).children().css({height: '-webkit-fill-available', 'white-space': 'normal', 'max-height': tdHeight*mergeRecord[item].rowspan + patchHeight - 10, 'line-height': tdHeight*mergeRecord[item].rowspan + patchHeight - 10 + 'px'});
                        $fixRight.eq(i).children('[data-key="'+item+'"]').attr('rowspan', mergeRecord[item].rowspan).css({'position': 'static', 'height': tdHeight*mergeRecord[item].rowspan + patchHeight}).children().css({height: '-webkit-fill-available', 'white-space': 'normal', 'max-height': tdHeight*mergeRecord[item].rowspan + patchHeight - 10, 'line-height': tdHeight*mergeRecord[item].rowspan + patchHeight - 10 + 'px'});
