package com.lefancrm.backend.dto;

import java.util.List;

/**
 * Created by lixianfeng on 2019/7/15.
 */
public class TemplateDataZY {
    private String entrustOrgName;
    private String entrustUserName;
    private String trusteeOrgName;
    private String surveyCno;
    private String entrustDate;
    private String safeTypeStr;
    private String safeDate;
    private String safeUserDate;
    private String safeUserAddress;
    private String surveyItem;
    private String accidentInfo;
    private String sexStr;
    private String nation;
    private String age;
    private String idNumber;
    private String numberTypeStr;
    private String domicileAddress;
    private List<SurveyCaseDirectionDto> directions;
    private String directionResult;
    private String directionBasis;
    private String names;
    private String qfUserName;
    private String datestr;
    private String directionNames;

    public String getEntrustOrgName() {
        return entrustOrgName;
    }

    public void setEntrustOrgName(String entrustOrgName) {
        this.entrustOrgName = entrustOrgName;
    }

    public String getEntrustUserName() {
        return entrustUserName;
    }

    public void setEntrustUserName(String entrustUserName) {
        this.entrustUserName = entrustUserName;
    }

    public String getTrusteeOrgName() {
        return trusteeOrgName;
    }

    public void setTrusteeOrgName(String trusteeOrgName) {
        this.trusteeOrgName = trusteeOrgName;
    }

    public String getSurveyCno() {
        return surveyCno;
    }

    public String getDomicileAddress() {
        return domicileAddress;
    }

    public void setDomicileAddress(String domicileAddress) {
        this.domicileAddress = domicileAddress;
    }

    public void setSurveyCno(String surveyCno) {
        this.surveyCno = surveyCno;
    }

    public String getEntrustDate() {
        return entrustDate;
    }

    public void setEntrustDate(String entrustDate) {
        this.entrustDate = entrustDate;
    }

    public String getSafeTypeStr() {
        return safeTypeStr;
    }

    public void setSafeTypeStr(String safeTypeStr) {
        this.safeTypeStr = safeTypeStr;
    }

    public String getSafeDate() {
        return safeDate;
    }

    public void setSafeDate(String safeDate) {
        this.safeDate = safeDate;
    }

    public String getSafeUserDate() {
        return safeUserDate;
    }

    public void setSafeUserDate(String safeUserDate) {
        this.safeUserDate = safeUserDate;
    }

    public String getSafeUserAddress() {
        return safeUserAddress;
    }

    public void setSafeUserAddress(String safeUserAddress) {
        this.safeUserAddress = safeUserAddress;
    }

    public String getSurveyItem() {
        return surveyItem;
    }

    public void setSurveyItem(String surveyItem) {
        this.surveyItem = surveyItem;
    }

    public String getAccidentInfo() {
        return accidentInfo;
    }

    public void setAccidentInfo(String accidentInfo) {
        this.accidentInfo = accidentInfo;
    }

    public String getSexStr() {
        return sexStr;
    }

    public void setSexStr(String sexStr) {
        this.sexStr = sexStr;
    }

    public String getNation() {
        return nation;
    }

    public void setNation(String nation) {
        this.nation = nation;
    }

    public String getIdNumber() {
        return idNumber;
    }

    public void setIdNumber(String idNumber) {
        this.idNumber = idNumber;
    }

    public List<SurveyCaseDirectionDto> getDirections() {
        return directions;
    }

    public void setDirections(List<SurveyCaseDirectionDto> directions) {
        this.directions = directions;
    }

    public String getDirectionResult() {
        return directionResult;
    }

    public void setDirectionResult(String directionResult) {
        this.directionResult = directionResult;
    }

    public String getDirectionBasis() {
        return directionBasis;
    }

    public void setDirectionBasis(String directionBasis) {
        this.directionBasis = directionBasis;
    }

    public String getNames() {
        return names;
    }

    public void setNames(String names) {
        this.names = names;
    }

    public String getQfUserName() {
        return qfUserName;
    }

    public void setQfUserName(String qfUserName) {
        this.qfUserName = qfUserName;
    }

    public String getDatestr() {
        return datestr;
    }

    public void setDatestr(String datestr) {
        this.datestr = datestr;
    }

    public String getAge() {
        return age;
    }

    public void setAge(String age) {
        this.age = age;
    }

    public String getDirectionNames() {
        return directionNames;
    }

    public void setDirectionNames(String directionNames) {
        this.directionNames = directionNames;
    }

    public String getNumberTypeStr() {
        return numberTypeStr;
    }

    public void setNumberTypeStr(String numberTypeStr) {
        this.numberTypeStr = numberTypeStr;
    }
}
