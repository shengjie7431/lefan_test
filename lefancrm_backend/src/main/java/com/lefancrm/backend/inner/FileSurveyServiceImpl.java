 ...
 */
@Service
public class FileSurveyServiceImpl implements FileSurveyService {
	...
	@Deprecated
	@Override
	public List<UserFileDto> uploadFile(MultipartFile file,String userFileDescription, String filePath, Long userId, List<FileItem> fileItemList) {
...
//            }
			...
			userFile.setUserId(userId);
			userFile.setFileName(fileName);
            userFile.setFullFileName(file.getOriginalFilename());
			userFile.setUserFileDescription(userFileDescription);
			userFile.setFinalName(newFileName);
			userFile.setUserFilePath(filePath + newFileName);
			userFile.setUserFileSize(file.getSize());
			userFile.setUserFileType(fileExt);
			userFile.setContentType(contentType);
			userFile.setImageSize(ImageSizeEnum.SIZE_DEFAULT.getValue());
			try {
