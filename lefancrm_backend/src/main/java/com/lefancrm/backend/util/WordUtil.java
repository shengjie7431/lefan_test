...
//            configuration.setClassForTemplateLoading(WordUtil.class,sourcePath);
    ...
    public static void main(String[] args) {
        ...
        String ybSurveyNo = "1286H18257223612240384130101";
        System.out.println(ybSurveyNo.substring(4,13));
        System.out.println(ybSurveyNo.substring(14,25));
        if (true){
            return;
        }
...
//                managerUserImg = "https://ddrapi.shlefan.com/sftp/files/test//ddr/cno/cwt1051578360084097/material/2.jpg";
        ...
        if (type == 1 || type == 6 || type == 7){
            ...
            if (data.getSurveyRiskCaseInfo().getSurveyRiskCase().getHzContactName() == null){
                map.put("hzContactName",template.getEntrustUserName());//投保人
            }else{
                map.put("hzContactName",data.getSurveyRiskCaseInfo().getSurveyRiskCase().getHzContactName());//投保人
            }




            //2024年3月4日  友邦人寿保险有限公司  换模板样式
            try {
                if (data.getSurveyRiskCaseInfo().getSurveyConsignor().getId().toString().equals("200")){
                    Double claimMoney = data.getSurveyRiskCaseInfo().getSurveyRiskCase().getClaimsMoney();
                    map.put("claimMoney",claimMoney == null ? 0L : claimMoney.longValue());
                    templateName = "lefan-stamp-yb.ftl";

                    map.put("ybCaseNo","");//理赔案件号
                    map.put("ybSurveyNo","");//调查号
                    //友邦   理赔案件号  调查号  保单号
                    String ybSurveyNo = data.getSurveyRiskCaseInfo().getSurveyRiskCase().getHzContactName();
                    if (ybSurveyNo != null){
                        if (ybSurveyNo.length() == 28){
                            map.put("surveyCno",ybSurveyNo.substring(4,14));//保单号
                            map.put("ybCaseNo",ybSurveyNo.substring(14,26));//理赔案件号
                            map.put("ybSurveyNo",ybSurveyNo);//调查号
                        }
                    }
