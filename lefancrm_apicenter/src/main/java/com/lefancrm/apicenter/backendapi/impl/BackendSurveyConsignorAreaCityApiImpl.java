...
import com.lefancrm.apicenter.backendapi.BackendSurveyConsignorAreaCityApi;
import com.lefancrm.apicenter.dao.CommonAreaMapper;
import com.lefancrm.apicenter.dao.SurveyConsignorAreaCityMapper;
...
public class BackendSurveyConsignorAreaCityApiImpl extends BaseServiceImpl implements BackendSurveyConsignorAreaCityApi {
    @Autowired
    private CommonAreaMapper commonAreaMapper;
    @Autowired
    private SurveyConsignorAreaCityMapper surveyConsignorAreaCityMapper;

    @ApiMethod(descript = "获取委托方区域列表", value = "backend-list-survey-consignor-area-city", apiParams = { })
    @Override
    public ApiResponse getAreaData(ApiRequest apiRequest) {
        Long orgId = apiRequest.getLong("id");
        Map<String,Object> paramMap =  new HashMap<String,Object>();
        paramMap.put("orgId",orgId);
        List<SurveyConsignorAreaCity> areas = surveyConsignorAreaCityMapper.getAreas(paramMap);
        return new ApiResponse(ApiMsgEnum.SUCCESS,areas.size(),areas);
    }


    @ApiMethod(descript = "获取委托方区域列表", value = "backend-list-survey-consignor-area-city-common", apiParams = { })
    @Override
    public ApiResponse getCommonAreas(ApiRequest apiRequest){
        ...
        for (CommonArea commonArea : commonAreas) {
            ...
            commonArea.setSelectedChildrenNum(surveyConsignorAreaCityMapper.selectAllSelectedCount(collect,orgId));
