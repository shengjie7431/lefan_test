package com.lefancrm.apicenter.util.wechatPay.client;
...
public class TenpayHttpClient {
    ...
    private static final String JKS_CA_FILENAME = "apiclient_cert.p12";

    private static final String JKS_CA_ALIAS = "tenpay";

    private static final String JKS_CA_PASSWORD = "1232622202";
     ...
     * @throws Exception
     */
    public static String postTFMSSL(String sslUrl, String xmlParam,String path) throws Exception {
        KeyStore keyStore  = KeyStore.getInstance("PKCS12");
        System.out.println("************************************************* path=" + path+ WxPubPayConfig.wxpaycertPath);
        FileInputStream instream = new FileInputStream(new File(path+ WxPubPayConfig.wxpaycertPath));
        try {
            keyStore.load(instream, WxPubPayConfig.partner.toCharArray());
        } finally {
            instream.close();
        }
        // Trust own CA and all self-signed certs
        SSLContext sslcontext = SSLContexts.custom()
                .loadKeyMaterial(keyStore, WxPubPayConfig.partner.toCharArray())
                .build();
        ...
        try {
            ...
            System.out.println("************************************************* sslUrl=" + sslUrl);
            //传入XML参数
            ...
            try {
                ...
                if (entity != null) {
                    ...
                    List<String> lines = IOUtils.readLines(ins, Charset.forName("UTF-8"));
    ...
    public boolean callHttpPost(String url, String postdata) {
        ...
        byte[] postData;
        try {
            postData = postdata.getBytes("UTF8");
            this.httpPostMethod(url, postData);
            flag = true;
    ...
    public static String postOfMSSL(String sslUrl, String xmlParam,String path) throws Exception {
        KeyStore keyStore = KeyStore.getInstance("PKCS12");
        System.out.println("************************************************* path=" + path + WxPayConfig.wxpaycertPath);
        FileInputStream instream = new FileInputStream(new File(path + WxPayConfig.wxpaycertPath));
        try {
        ...
        // Trust own CA and all self-signed certs
        SSLContext sslcontext = SSLContexts.custom()
                .loadKeyMaterial(keyStore, WxPayConfig.partner.toCharArray())
                .build();
        // Allow TLSv1 protocol only
        SSLConnectionSocketFactory sslsf = new SSLConnectionSocketFactory(
                sslcontext,
